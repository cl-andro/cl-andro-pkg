...
CLANDRO_PKG_HOMEPAGE=https://www.rust-lang.org/
CLANDRO_PKG_DESCRIPTION="Systems programming language focused on safety, speed and concurrency"
CLANDRO_PKG_LICENSE="MIT"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION="1.95.0"
CLANDRO_PKG_SRCURL=https://static.rust-lang.org/dist/rustc-${CLANDRO_PKG_VERSION}-src.tar.xz
CLANDRO_PKG_SHA256=62b67230754da642a264ca0cb9fc08820c54e2ed7b3baba0289876d4cdb48c08
CLANDRO_PKG_DEPENDS="clang, libandroid-execinfo, libc++, libllvm (<< $CLANDRO_LLVM_NEXT_MAJOR_VERSION), lld, openssl, zlib"
CLANDRO_PKG_BUILD_DEPENDS="wasi-libc"
CLANDRO_PKG_SUGGESTS="rust-analyzer"
CLANDRO_PKG_NO_REPLACE_GUESS_SCRIPTS=true
...
clandro_pkg_auto_update() {
	...
	sed \
		-e "s/^\tlocal BOOTSTRAP_VERSION=.*/\tlocal BOOTSTRAP_VERSION=${CLANDRO_PKG_VERSION}/" \
...
clandro_step_pre_configure() {
	...
	CLANDRO_PKG_DEPENDS+=", rust-std-${CARGO_TARGET_NAME/_/-}"

	local p="${CLANDRO_PKG_BUILDER_DIR}/0001-set-CLANDRO_PKG_API_LEVEL.diff"
	echo "Applying patch: $(basename "${p}")"
	sed "s|@CLANDRO_PKG_API_LEVEL@|${CLANDRO_PKG_API_LEVEL}|g" "${p}" \
		| patch --silent -p1

	# assist with downstream patch methods that bulk-replace
	# string 'com.zk.clandro' throughout the repository
	...
	if [[ "$CLANDRO_PREFIX" != "$original_prefix" ]]; then
		local patch="$CLANDRO_PKG_BUILDER_DIR/force-allow-edit-vendor.diff"
		echo "Applying patch: $(basename "$patch")"
		patch --silent -p1 < "$patch"

		local crate=openssl-probe
		local crate_src_dir="$(realpath "$(find "$CLANDRO_PKG_SRCDIR/vendor" -name "$crate"'*' | sort | tail -n1)")"
		local crate_dest_dir="$crate_src_dir-custom-termux-prefix"
		cp -r "$crate_src_dir" "$crate_dest_dir"

		echo "Replacing '$original_prefix' with '$CLANDRO_PREFIX' in '$crate_dest_dir/src/lib.rs'"
		sed -i -e "s|$original_prefix|$CLANDRO_PREFIX|g" "$crate_dest_dir/src/lib.rs"

		local dir
		for dir in "$CLANDRO_PKG_SRCDIR"/{,src/tools/cargo,src/tools/miri}; do
			echo '' >> "$dir/Cargo.toml"
	...
	#	ln -vfst $RUST_LIBDIR \
	#		$CLANDRO_STANDALONE_TOOLCHAIN/sysroot/usr/lib/$CLANDRO_HOST_PLATFORM/$CLANDRO_PKG_API_LEVEL/lib{c,dl}.so
	# but written in a future-proof manner.
	ln -vfst $RUST_LIBDIR $(echo | $CC -x c - -Wl,-t -shared | grep '\.so$')

	ln -vfst "${RUST_LIBDIR}" "${CLANDRO_PREFIX}"/lib/libandroid-execinfo.so
}
...
clandro_step_configure() {
	...
	# rust 1.89.0
	export WASI_SDK_PATH="${CLANDRO_PKG_TMPDIR}/wasi-sdk"
	rm -fr "${WASI_SDK_PATH}"
	mkdir -p "${WASI_SDK_PATH}"/{bin,share}
	ln -fsv "${CLANDRO_PREFIX}/share/wasi-sysroot" "${WASI_SDK_PATH}/share/wasi-sysroot"
	local clang
	...
	if [[ "${CLANDRO_ON_DEVICE_BUILD}" == "true" ]]; then
		local dir="${CLANDRO_STANDALONE_TOOLCHAIN}/toolchains/llvm/prebuilt/linux-x86_64/bin"
		mkdir -p "${dir}"
		local target clang
		for target in aarch64-linux-android armv7a-linux-androideabi i686-linux-android x86_64-linux-android; do
			for clang in clang clang++; do
				ln -fsv "${CLANDRO_PREFIX}/bin/clang" "${dir}/${target}${CLANDRO_PKG_API_LEVEL}-${clang}"
			done
		done
	fi
	...
	sed \
		...
		-e "s|@CLANDRO_HOST_LLVM_BASE_DIR@|${CLANDRO_HOST_LLVM_BASE_DIR}|g" \
		-e "s|@CARGO_TARGET_NAME@|${CARGO_TARGET_NAME}|g" \
		-e "s|@RUSTC@|${RUSTC}|g" \
		-e "s|@CARGO@|${CARGO}|g" \
		"${CLANDRO_PKG_BUILDER_DIR}"/bootstrap.toml > bootstrap.toml

	local env_host=$(printf $CARGO_TARGET_NAME | tr a-z A-Z | sed s/-/_/g)
	export ${env_host}_OPENSSL_DIR=$CLANDRO_PREFIX
	export RUST_LIBDIR=$CLANDRO_PKG_BUILDDIR/_lib
	...
	"${CC}" ${CPPFLAGS} -c "${CLANDRO_PKG_BUILDER_DIR}/syncfs.c"
	...
	export CARGO_TARGET_${env_host}_RUSTFLAGS+=" -C link-arg=-Wl,-rpath=${CLANDRO_PREFIX}/lib -C link-arg=-Wl,--enable-new-dtags"
...
clandro_step_make_install() {
	...
	local job="install"
	[[ "${CLANDRO_ON_DEVICE_BUILD}" == "true" ]] && job="dist"

	# rust 1.87.0
	# https://github.com/termux/termux-packages/issues/25360
	# build to stage 2 to fix rust-analyzer error
	"${CLANDRO_PKG_SRCDIR}/x.py" "${job}" -j "${CLANDRO_PKG_MAKE_PROCESSES}" --stage 2

	# wasm32* not added into bootstrap.toml
	# due to CI and on device build error:
	# error: could not document `std`
	"${CLANDRO_PKG_SRCDIR}/x.py" install -j "${CLANDRO_PKG_MAKE_PROCESSES}" --target wasm32-unknown-unknown --stage 2 std
	[[ ! -e "${CLANDRO_PREFIX}/share/wasi-sysroot" ]] && clandro_error_exit "wasi-sysroot not found"
	"${CLANDRO_PKG_SRCDIR}/x.py" install -j "${CLANDRO_PKG_MAKE_PROCESSES}" --target wasm32-wasip1 --stage 2 std
	"${CLANDRO_PKG_SRCDIR}/x.py" install -j "${CLANDRO_PKG_MAKE_PROCESSES}" --target wasm32-wasip2 --stage 2 std
	"${CLANDRO_PKG_SRCDIR}/x.py" install -j "${CLANDRO_PKG_MAKE_PROCESSES}" --target wasm32-wasip3 --stage 2 std

	"${CLANDRO_PKG_SRCDIR}/x.py" dist -j "${CLANDRO_PKG_MAKE_PROCESSES}" --stage 2 rustc-dev

	# remove version suffix: beta, nightly
	local VERSION=${CLANDRO_PKG_VERSION//~*}

	if [[ "${CLANDRO_ON_DEVICE_BUILD}" == "true" ]]; then
		echo "WARN: Replacing on device rust! Caveat emptor!"
		rm -fr ${CLANDRO_PREFIX}/lib/rustlib/${CARGO_TARGET_NAME}
		rm -fv $(find ${CLANDRO_PREFIX}/lib -maxdepth 1 -type l -exec ls -l "{}" \; | grep rustlib | sed -e "s|.* ${CLANDRO_PREFIX}/lib|${CLANDRO_PREFIX}/lib|" -e "s| -> .*||")
	fi
	ls build/dist/*-${VERSION}*.tar.gz | xargs -P${CLANDRO_PKG_MAKE_PROCESSES} -n1 -t -r tar -xf
	local tgz
	for tgz in $(ls build/dist/*-${VERSION}*.tar.gz); do
		echo "INFO: ${tgz}"
		./$(basename "${tgz}" | sed -e "s|.tar.gz$||")/install.sh --prefix=${CLANDRO_PREFIX}
	done
	...
	ln -vfs lld ${CLANDRO_PREFIX}/bin/rust-lld

	cd "$CLANDRO_PREFIX/lib/rustlib"
	rm -fr \
		components \
		install.log \
		uninstall.sh \
	...
	cd "${CLANDRO_PREFIX}/lib/rustlib/${CARGO_TARGET_NAME}/lib"
	echo "INFO: ${CLANDRO_PKG_BUILDDIR}/rustlib-rlib.txt"
	ls *.rlib | tee "${CLANDRO_PKG_BUILDDIR}/rustlib-rlib.txt"

	echo "INFO: ${CLANDRO_PKG_BUILDDIR}/rustlib-so.txt"
	ls *.so | tee "${CLANDRO_PKG_BUILDDIR}/rustlib-so.txt"

	echo "INFO: ${CLANDRO_PKG_BUILDDIR}/rustc-dev-${VERSION}-${CARGO_TARGET_NAME}/rustc-dev/manifest.in"
	cat "${CLANDRO_PKG_BUILDDIR}/rustc-dev-${VERSION}-${CARGO_TARGET_NAME}/rustc-dev/manifest.in" | tee "${CLANDRO_PKG_BUILDDIR}/manifest.in"

	sed -e 's/^.....//' -i "${CLANDRO_PKG_BUILDDIR}/manifest.in"
	local _included=$(cat "${CLANDRO_PKG_BUILDDIR}/manifest.in")
