CLANDRO_PKG_HOMEPAGE=https://oxc.rs/
CLANDRO_PKG_DESCRIPTION="Oxc JavaScript linter"
CLANDRO_PKG_LICENSE="MIT"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION="1.63.0"
CLANDRO_PKG_SRCURL="https://github.com/oxc-project/oxc/archive/refs/tags/oxlint_v$CLANDRO_PKG_VERSION.tar.gz"
CLANDRO_PKG_SHA256=ac6d35c497ef86c258f324f6fd5acf2dbf81eadcd8659259634c972a16f37db6
CLANDRO_PKG_AUTO_UPDATE=true
CLANDRO_PKG_BUILD_IN_SRC=true

clandro_pkg_auto_update() {
	# based on `clandro_github_api_get_tag.sh`
	# fetch newest tags
	local newest_tags newest_tag
	newest_tags="$(curl -d "$(cat <<-EOF | tr '\n' ' '
	{
		"query": "query {
			repository(owner: \"oxc-project\", name: \"oxc\") {
				refs(refPrefix: \"refs/tags/\", first: 20, orderBy: {
					field: TAG_COMMIT_DATE, direction: DESC
				})
				{ edges { node { name } } }
			}
		}"
	}
	EOF
	)" \
		-H "Authorization: token ${GITHUB_TOKEN}" \
		-H "Accept: application/vnd.github.v3+json" \
		--silent \
		--location \
		--retry 10 \
		--retry-delay 1 \
		https://api.github.com/graphql \
		| jq '.data.repository.refs.edges[].node.name')"
	# filter only tags having "oxlint_v" and extract only raw version.
	read -r newest_tag < <(echo "$newest_tags" | grep -Po 'oxlint_v\K\d+\.\d+\.\d+' | sort -Vr)

	[[ -z "${newest_tag}" ]] && clandro_error_exit "Unable to get tag from ${CLANDRO_PKG_SRCURL}"
	clandro_pkg_upgrade_version "${newest_tag}"
}

clandro_step_pre_configure() {
	clandro_setup_rust
	clandro_setup_nodejs
	clandro_setup_cmake
	# Dummy CMake toolchain file to workaround build error:
	# error: failed to run custom build command for `libz-ng-sys v1.1.15`
	# ...
	# CMake Error at /home/builder/.termux-build/_cache/cmake-3.28.3/share/cmake-3.28/Modules/Platform/Android-Determine.cmake:217 (message):
	# Android: Neither the NDK or a standalone toolchain was found.
	export TARGET_CMAKE_TOOLCHAIN_FILE="$CLANDRO_PKG_BUILDDIR/android.toolchain.cmake"
	touch "$TARGET_CMAKE_TOOLCHAIN_FILE"

	: "${CARGO_HOME:=$HOME/.cargo}"
	export CARGO_HOME
	cargo fetch --target "$CARGO_TARGET_NAME"

	# ld.lld: error: undefined symbol: __atomic_load_8
	if [[ "$CLANDRO_ARCH" == "i686" ]]; then
		local -u env_host="${CARGO_TARGET_NAME//-/_}"
		export CARGO_TARGET_${env_host}_RUSTFLAGS+=" -C link-arg=$(${CC} -print-libgcc-file-name)"
	fi
}

clandro_step_make() {
	cargo build --jobs "$CLANDRO_PKG_MAKE_PROCESSES" --target "$CARGO_TARGET_NAME" --release --all-features
}

clandro_step_make_install() {
	install -Dm700 -t "$CLANDRO_PREFIX/bin" "target/$CARGO_TARGET_NAME/release/oxlint"
	install -Dm700 -t "$CLANDRO_PREFIX/bin" "target/$CARGO_TARGET_NAME/release/oxfmt"
}
