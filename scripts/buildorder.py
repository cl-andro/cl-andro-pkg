...
clandro_arch = os.getenv('CLANDRO_ARCH') or 'aarch64'
clandro_global_library = os.getenv('CLANDRO_GLOBAL_LIBRARY') or 'false'
clandro_pkg_library = os.getenv('CLANDRO_PACKAGE_LIBRARY') or 'bionic'
        ...
        for line in build_script:
            if line.startswith(vars):
                ...
                for dependency_value in re.split(',|\\|', dependencies_string):
                    ...
                    arch = os.getenv('CLANDRO_ARCH')
                    if arch is None:
                        arch = 'aarch64'
                    if arch == "x86_64":
                        arch = "x86-64"
                    dependency_value = re.sub(r'\${CLANDRO_ARCH/_/-}', arch, dependency_value)
...
def parse_build_file_dependencies(path):
    ...
    return parse_build_file_dependencies_with_vars(path, ('CLANDRO_PKG_DEPENDS', 'CLANDRO_PKG_BUILD_DEPENDS', 'CLANDRO_SUBPKG_DEPENDS', 'CLANDRO_PKG_DEVPACKAGE_DEPENDS'))

def parse_build_file_antidependencies(path):
    "Extract the antidependencies of a build.sh file."
    return parse_build_file_dependencies_with_vars(path, 'CLANDRO_PKG_ANTI_BUILD_DEPENDS')
...
def parse_build_file_excluded_arches(path):
    ...
    with open(path, encoding="utf-8") as build_script:
        for line in build_script:
            if line.startswith(('CLANDRO_PKG_EXCLUDED_ARCHES', 'CLANDRO_SUBPKG_EXCLUDED_ARCHES')):
...
class ClandroPackage(object):
    ...
    def __init__(self, dir_path, fast_build_mode):
        ...
        self.only_installing = parse_build_file_variable_bool(build_sh_path, 'CLANDRO_PKG_ONLY_INSTALLING')
        self.separate_subdeps = parse_build_file_variable_bool(build_sh_path, 'CLANDRO_PKG_SEPARATE_SUB_DEPENDS')
        self.accept_dep_scr = parse_build_file_variable_bool(build_sh_path, 'CLANDRO_PKG_ACCEPT_PKG_IN_DEP')

        if os.getenv('CLANDRO_ON_DEVICE_BUILD') == "true" and clandro_pkg_library == "bionic":
            always_deps = ['libc++']
        ...
        for filename in os.listdir(self.dir):
            if not filename.endswith('.subpackage.sh'):
                continue
            subpkg = ClandroSubPackage(self.dir + '/' + filename, self)
            if clandro_arch in subpkg.excluded_arches:
                continue

            self.subpkgs.append(subpkg)

        subpkg = ClandroSubPackage(self.dir + '/' + self.name + '-static' + '.subpackage.sh', self, virtual=True)
        self.subpkgs.append(subpkg)
    ...
    def recursive_dependencies(self, pkgs_map, dir_root=None):
        ...
        for dependency_name in sorted(self.deps):
            if clandro_global_library == "true" and clandro_pkg_library == "glibc" and not has_prefix_glibc(dependency_name):
...
def read_packages_from_directories(directories, fast_build_mode, full_buildmode):
    """Construct a map from package name to ClandroPackage.
    ...
    for package_dir in directories:
        for pkgdir_name in sorted(os.listdir(package_dir)):
            ...
            if os.path.isfile(dir_path + '/build.sh'):
                new_package = ClandroPackage(package_dir + '/' + pkgdir_name, fast_build_mode)

                if clandro_arch in new_package.excluded_arches:
                    continue
    ...
    for pkg in all_packages:
        for dependency_name in pkg.deps:
            ...
            if fast_build_mode or not isinstance(pkg, ClandroSubPackage):
...
def generate_full_buildorder(pkgs_map):
    ...
    # List of all ClandroPackages without dependencies
...
def main():
    ...
    for pkg in build_order:
        pkg_name = pkg.name
        if clandro_global_library == "true" and clandro_pkg_library == "glibc" and not has_prefix_glibc(pkg_name):
