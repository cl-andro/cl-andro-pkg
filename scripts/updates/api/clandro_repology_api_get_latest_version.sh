...
#
# But hopefully, all this can be avoided if CLANDRO_PKG_UPDATE_VERSION_REGEXP is set.
#
clandro_repology_api_get_latest_version() {
	if [[ -z "$1" ]]; then
		clandro_error_exit "Usage: ${FUNCNAME[0]} PKG_NAME"
	fi

	# Why `--arg`? See: https://stackoverflow.com/a/54674832/15086226; `sub` strips the leading 'v'
	jq -r --arg pkg "$1" '.[$pkg] // "null" | sub("^v";"")' "$CLANDRO_REPOLOGY_DATA_FILE"
}
