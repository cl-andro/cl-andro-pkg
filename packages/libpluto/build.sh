CLANDRO_PKG_HOMEPAGE=https://plutolang.github.io/
CLANDRO_PKG_DESCRIPTION="Shared library for the Pluto interpreter"
CLANDRO_PKG_LICENSE="MIT"
CLANDRO_PKG_MAINTAINER="Komo @mbekkomo"
CLANDRO_PKG_VERSION="0.12.2"
CLANDRO_PKG_SRCURL="https://github.com/PlutoLang/Pluto/archive/refs/tags/${CLANDRO_PKG_VERSION}.tar.gz"
CLANDRO_PKG_SHA256=69ad5fa99e65f824229c025110bf52d9bf0c28a6df565cb0f25826a1bb2acda2
CLANDRO_PKG_AUTO_UPDATE=true
CLANDRO_PKG_DEPENDS="libc++"
CLANDRO_PKG_BUILD_DEPENDS="dos2unix, readline"
CLANDRO_PKG_BUILD_IN_SRC=true
CLANDRO_PKG_BREAKS="plutolang (<< 0.9.4-1)"
CLANDRO_PKG_REPLACES="plutolang (<< 0.9.4-1)"

clandro_step_post_get_source() {
	# robertkirkman: CRLF drives me absolutely up the wall, sorry I can't work with it.
	# if another libpluto maintainer prefers CRLF, please feel free to redo everything I do in CRLF instead.
	if [[ "$CLANDRO_ON_DEVICE_BUILD" == "false" ]]; then
		DOS2UNIX="$CLANDRO_PKG_TMPDIR/dos2unix"
		(. "$CLANDRO_SCRIPTDIR/packages/dos2unix/build.sh"; CLANDRO_PKG_SRCDIR="$DOS2UNIX" clandro_step_get_source)
		pushd "$DOS2UNIX"
		make dos2unix
		popd # DOS2UNIX
		export PATH="$DOS2UNIX:$PATH"
	fi

	find "$CLANDRO_PKG_SRCDIR" -type f -print0 | xargs -0 dos2unix
}

clandro_step_configure() {
	# can't use php version from termux repo, there's no build for the latest version on the pre-built binaries
	local php_version="8.3.9"
	local tarball_checksum="6c7b45dabc362c9c447c6c9688f8922d0cfa3d5c4618d632c504342d37085e2f"

	mkdir -p "${CLANDRO_PKG_CACHEDIR}/php"
	clandro_download "https://dl.static-php.dev/static-php-cli/common/php-${php_version}-cli-linux-x86_64.tar.gz" \
		"${CLANDRO_PKG_CACHEDIR}/php.tar.gz" \
		"${tarball_checksum}"
	tar -zxf "${CLANDRO_PKG_CACHEDIR}/php.tar.gz" -C "${CLANDRO_PKG_CACHEDIR}/php"
	chmod +x "${CLANDRO_PKG_CACHEDIR}/php/php"
	export PATH="${CLANDRO_PKG_CACHEDIR}/php:$PATH"
}

clandro_step_make() {
	local build linkreadline

	export TARGET_ARCH="${CLANDRO_ARCH}"
	CXXFLAGS+=" -DLUA_USE_LINUX -DLUA_USE_READLINE"

	CXXFLAGS="${CPPFLAGS} ${CXXFLAGS}" php scripts/compile.php "${CXX}"
	for build in 'shared' 'static' 'pluto' 'plutoc'; do
		linkreadline=""
		[[ "$build" == "pluto" ]] && linkreadline="-lreadline"
		# ANDROID_ROOT is just a dummy variable, it's required to detect Android platform
		ANDROID_ROOT=/system LDFLAGS="${LDFLAGS} ${linkreadline}" php scripts/link_"$build".php "${CXX}"
	done
}

clandro_step_make_install() {
	install -Dm755 -t "${CLANDRO_PREFIX}/bin" src/pluto{,c}
	install -Dm644 -t "${CLANDRO_PREFIX}/lib" src/libpluto{.so,static.a}
	install -Dm644 -t "${CLANDRO_PREFIX}/include/pluto" src/{lua,lauxlib,lualib,luaconf}.h src/lua.hpp
}
