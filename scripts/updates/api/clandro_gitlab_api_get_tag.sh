...
clandro_gitlab_api_get_tag() {
	...
	tag_type="$CLANDRO_PKG_UPDATE_TAG_TYPE"

	# Example:
	# https://gitlab.freedesktop.org/xorg/app/xeyes/-/archive/xeyes-${CLANDRO_PKG_VERSION}/xeyes-xeyes-${CLANDRO_PKG_VERSION}.tar.gz
	#        _="https:"
	#        _=""
	# api_host="gitlab.freedesktop.org"
	#  project="xorg/app/xeyes/-/archive/xeyes-${CLANDRO_PKG_VERSION}/xeyes-xeyes-${CLANDRO_PKG_VERSION}.tar.gz"
	IFS='/' read -r _ _ api_host project <<< "${CLANDRO_PKG_SRCURL}"
	...
	if [[ -z "${tag_type}" ]]; then # If not set, then decide on the basis of url.
		...
		elif [[ -n "$CLANDRO_PKG_UPDATE_VERSION_REGEXP" ]]; then
	...
	case "${tag_type}" in
		...
		*)
			clandro_error_exit <<-EndOfError
				ERROR: Invalid CLANDRO_PKG_UPDATE_TAG_TYPE: '${tag_type}'.
