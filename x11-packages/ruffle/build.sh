CLANDRO_PKG_HOMEPAGE=https://ruffle.rs/
CLANDRO_PKG_DESCRIPTION="A Flash Player emulator written in Rust"
CLANDRO_PKG_LICENSE="MIT, Apache-2.0"
CLANDRO_PKG_LICENSE_FILE="LICENSE.md"
CLANDRO_PKG_MAINTAINER="@clandro"
_COMMIT_DATE=2026-05-01
CLANDRO_PKG_VERSION="0.0.1-nightly-2026-05-01"
CLANDRO_PKG_SRCURL=https://github.com/ruffle-rs/ruffle/archive/refs/tags/nightly-${_COMMIT_DATE}.tar.gz
CLANDRO_PKG_SHA256=7b2015b51a714670197d18f16feef2b48d8da257a36458335aa9752eda64039c
CLANDRO_PKG_DEPENDS="alsa-lib, alsa-plugins, fontconfig, gtk3, openh264"
...
clandro_pkg_auto_update() {
	local latest_tags
	latest_tags="$(clandro_github_api_get_tag)"
...
clandro_step_pre_configure() {
	...
	patch="$CLANDRO_PKG_BUILDER_DIR/fontdb-load-fonts.diff"
	dir="vendor/fontdb"
	echo "Applying patch: $patch"
	patch --silent -p1 -d "$dir" < "${patch}"
	...
	patch="$CLANDRO_PKG_BUILDER_DIR/wgpu-hal-no-khr-display.diff"
	dir="vendor/wgpu-hal"
	echo "Applying patch: $patch"
	patch --silent -p1 -d "$dir" < "${patch}"
...
clandro_step_make_install() {
	install -Dm755 \
		$CLANDRO_PKG_SRCDIR/target/${CARGO_TARGET_NAME}/release/ruffle_desktop \
		$CLANDRO_PREFIX/bin/ruffle

	install -Dm0644 \
		-t "$CLANDRO_PREFIX"/share/doc/"$CLANDRO_PKG_NAME" \
		"$CLANDRO_PKG_BUILDDIR"/README.md

	local s
	for s in 16x16 32x32 128x128 256x256 512x512; do
		install -Dm0644 \
			"desktop/assets/Assets.xcassets/RuffleMacIcon.iconset/icon_$s.png" \
			"$CLANDRO_PREFIX/share/icons/hicolor/$s/apps/ruffle.png"
	done

	install -d "$CLANDRO_PREFIX/share/applications/"
	cat > "$CLANDRO_PREFIX/share/applications/ruffle.desktop" << _EOD
[Desktop Entry]
Type=Application
Name=Ruffle
GenericName=Ruffle
