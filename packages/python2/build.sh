CLANDRO_PKG_HOMEPAGE=http://python.org/
CLANDRO_PKG_DESCRIPTION="Python 2 programming language intended to enable clear programs"
CLANDRO_PKG_LICENSE="PythonPL"
CLANDRO_PKG_MAINTAINER="@clandro"
_MAJOR_VERSION=2.7
CLANDRO_PKG_VERSION=${_MAJOR_VERSION}.18
...
CLANDRO_PKG_SHA256=b62c0e7937551d0cc02b8fd5cb0f544f9405bafc9a54d3808ed4594812edef43
CLANDRO_PKG_DEPENDS="gdbm, libandroid-posix-semaphore, libandroid-support, libbz2, libcrypt, libffi, libsqlite, ncurses, ncurses-ui-libs, openssl, readline, zlib"
CLANDRO_PKG_RECOMMENDS="clang, make, pkg-config"
CLANDRO_PKG_BREAKS="python2-dev"
CLANDRO_PKG_REPLACES="python2-dev"
...
CLANDRO_PKG_EXTRA_CONFIGURE_ARGS+=" --build=$CLANDRO_BUILD_TUPLE --with-system-ffi --without-ensurepip"
CLANDRO_PKG_EXTRA_CONFIGURE_ARGS+=" --enable-unicode=ucs4"
...
clandro_step_host_build() {
	# We need a host-built Parser/pgen binary, copied into cross-compile build in clandro_step_post_configure() below
...
clandro_step_post_configure() {
	cp $CLANDRO_PKG_HOSTBUILD_DIR/Parser/pgen $CLANDRO_PKG_BUILDDIR/Parser/pgen
	touch -d "next hour" $CLANDRO_PKG_BUILDDIR/Parser/pgen
}

clandro_step_pre_configure() {
	# Put the host-built python in path:
	export CLANDRO_ORIG_PATH=$PATH
	export PATH=$CLANDRO_PKG_HOSTBUILD_DIR:$PATH

	if [ $CLANDRO_ARCH = i686 ] || [ $CLANDRO_ARCH = arm ]; then LDFLAGS+=" -lm"; fi
...
clandro_step_post_make_install() {
	...
	# Restore path which clandro_step_host_build messed with
	export PATH=$CLANDRO_ORIG_PATH
}
...
clandro_step_create_debscripts() {
	...
	echo "if [ ! -f $CLANDRO_PREFIX/bin/pip2 -a -d $CLANDRO_PREFIX/lib/python${_MAJOR_VERSION}/site-packages/pip-*.dist-info ]; then rm -Rf $CLANDRO_PREFIX/lib/python${_MAJOR_VERSION}/site-packages/pip-*.dist-info ; fi" >> postinst
	# Setup bin/pip2:
	echo "$CLANDRO_PREFIX/bin/python2 -m ensurepip --upgrade --no-default-pip" >> postinst

	## PRE RM:
	# Avoid running on update
	echo "#!$CLANDRO_PREFIX/bin/sh" > prerm
	echo "if [ \"$CLANDRO_PACKAGE_FORMAT\" = \"pacman\" ] && [ \"\$1\" != \"remove\" ]; then exit 0; fi" >> prerm
	# Uninstall everything installed through pip:
	echo "pip2 freeze 2> /dev/null | xargs pip2 uninstall -y > /dev/null 2> /dev/null" >> prerm
	# Cleanup *.pyc files
	echo "find $CLANDRO_PREFIX/lib/python${_MAJOR_VERSION} -depth -name *.pyc -exec rm -rf {} +" >> prerm
	# Remove contents of site-packages/ folder:
	echo "rm -Rf $CLANDRO_PREFIX/lib/python${_MAJOR_VERSION}/site-packages/*" >> prerm
	# Remove pip and easy_install installed by ensurepip in postinst:
	echo "rm -f $CLANDRO_PREFIX/bin/pip2* $CLANDRO_PREFIX/bin/easy_install-2*" >> prerm
