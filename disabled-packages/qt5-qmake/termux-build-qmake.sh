...
CLANDRO_PREFIX=/data/data/com.zk.clandro/files/usr
...
clandro_step_configure () {
    export PKG_CONFIG_SYSROOT_DIR="${CLANDRO_PREFIX}"
...
if [ ! -e "qtbase-everywhere-opensource-src-${CLANDRO_PKG_VERSION}.tar.xz" ]; then
    echo "[*] Downloading Qt sources..."
    curl -L --output "qtbase-everywhere-opensource-src-${CLANDRO_PKG_VERSION}.tar.xz" "${CLANDRO_PKG_SRCURL}"
fi

echo "[*] Unpacking Qt sources..."
rm -rf "qtbase-everywhere-src-${CLANDRO_PKG_VERSION}"
tar xf "qtbase-everywhere-opensource-src-${CLANDRO_PKG_VERSION}.tar.xz"

cd "qtbase-everywhere-src-${CLANDRO_PKG_VERSION}" && {
    ## Patch the source
...
cp "qtbase-everywhere-src-${CLANDRO_PKG_VERSION}/qmake/qmake" "./qmake-$(uname -m)-linux-android"
