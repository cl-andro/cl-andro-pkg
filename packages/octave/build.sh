...
CLANDRO_PKG_DESCRIPTION="GNU Octave is a high-level language, primarily intended for numerical computations. (only CLI)"
...
CLANDRO_PKG_DEPENDS="libandroid-complex-math, arpack-ng, clang, bzip2, fftw, fontconfig, freetype, ghostscript, glpk, graphicsmagick, libcurl, libhdf5, libiconv, libopenblas, libsndfile, make, openssl, pcre2, portaudio, qhull, qrupdate-ng, rapidjson, readline, suitesparse, sundials, zlib"
CLANDRO_PKG_BUILD_DEPENDS="gnuplot, less"
CLANDRO_PKG_RECOMMENDS="gnuplot, less"
CLANDRO_PKG_CONFLICTS="octave-x"
...
clandro_step_post_get_source() {
	...
	local ver_x=$(. $CLANDRO_SCRIPTDIR/x11-packages/octave-x/build.sh; echo ${CLANDRO_PKG_VERSION#*:})
	if [ "${ver_e}" != "${ver_x}" ]; then
