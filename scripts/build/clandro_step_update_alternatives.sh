...
clandro_parse_alternatives() {
	...
	while IFS=$'\n' read -r line; do
		...
		if (( dependents )); then
			...
			DEPENDENTS[${NAME[-1]}]+="      --slave \"${CLANDRO_PREFIX}/${dep_link}\" \"${dep_name}\" \"${CLANDRO_PREFIX}/${dep_alternative}\""$' \\\n'
		fi
...
clandro_step_update_alternatives() {
	...
	for alternatives_file in "${CLANDRO_PKG_BUILDER_DIR}"/*.alternatives; do
		[[ -f "$alternatives_file" ]] || continue
		local -a NAME=()
		local -A DEPENDENTS=() LINK=() ALTERNATIVE=() PRIORITY=()
		clandro_parse_alternatives "$alternatives_file"
		...
		[[ -f postinst.orig ]] && head -n1 postinst.orig || echo "#!${CLANDRO_PREFIX}/bin/sh"
		# Boilerplate header comment and checks
		echo "# Automatically added by clandro_step_update_alternatives"
		echo "if [ \"\$1\" = 'configure' ] || [ \"\$1\" = 'abort-upgrade' ] || [ \"\$1\" = 'abort-deconfigure' ] || [ \"\$1\" = 'abort-remove' ] || [ \"${CLANDRO_PACKAGE_FORMAT}\" = 'pacman' ]; then"
		echo "  if [ -x \"${CLANDRO_PREFIX}/bin/update-alternatives\" ]; then"
		...
		for name in "${NAME[@]}"; do
			...
			printf '%b' \
				"    # ${name}\n" \
				"    update-alternatives" $' \\\n' \
				"      --install \"${CLANDRO_PREFIX}/${LINK[$name]}\" \"${name}\" \"${CLANDRO_PREFIX}/${ALTERNATIVE[$name]}\" ${PRIORITY[$name]}"
		...
		[[ -f prerm.orig ]] && head -n1 prerm.orig || echo "#!${CLANDRO_PREFIX}/bin/sh"
		# Boilerplate header comment and checks
		echo "# Automatically added by clandro_step_update_alternatives"
		echo "if [ \"\$1\" = 'remove' ] || [ \"\$1\" != 'upgrade' ] || [ \"${CLANDRO_PACKAGE_FORMAT}\" = 'pacman' ]; then"
		echo "  if [ -x \"${CLANDRO_PREFIX}/bin/update-alternatives\" ]; then"
		# Remove each group
		for name in "${NAME[@]}"; do
			...
			# Removal line
			printf '%s\n' "    update-alternatives --remove \"${name}\" \"${CLANDRO_PREFIX}/${ALTERNATIVE[$name]}\""
		done
		# Close up boilerplate and add end comment
		echo "  fi"
		echo "fi"
