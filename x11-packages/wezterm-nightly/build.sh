...
CLANDRO_PKG_DESCRIPTION="GPU-accelerated cross-platform terminal emulator and multiplexer (development branch)"
CLANDRO_PKG_LICENSE="MIT"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION="20260331"
CLANDRO_PKG_SRCURL=git+https://github.com/wezterm/wezterm
...
CLANDRO_PKG_BREAKS="wezterm"
CLANDRO_PKG_CONFLICTS="wezterm"
...
clandro_pkg_auto_update() {
	...
	if [[ -z "${latest_commit_date}" ]]; then
		clandro_error_exit "Unable to get latest commit date from ${CLANDRO_PKG_SRCURL}"
	fi
	clandro_pkg_upgrade_version "${latest_commit_date}"
}
...
clandro_step_make_install() {
	install -Dm755 "target/${CARGO_TARGET_NAME}/release/${CLANDRO_PKG_NAME//-nightly/}" -t "${CLANDRO_PREFIX}/bin"
	install -Dm755 "target/${CARGO_TARGET_NAME}/release/${CLANDRO_PKG_NAME//-nightly/}-gui" -t "${CLANDRO_PREFIX}/bin"
	install -Dm755 "target/${CARGO_TARGET_NAME}/release/${CLANDRO_PKG_NAME//-nightly/}-mux-server" -t "${CLANDRO_PREFIX}/bin"
	install -Dm755 "target/${CARGO_TARGET_NAME}/release/strip-ansi-escapes" -t "${CLANDRO_PREFIX}/bin"
	install -Dm644 assets/icon/terminal.png "${CLANDRO_PREFIX}/share/icons/hicolor/128x128/apps/org.wezfurlong.${CLANDRO_PKG_NAME//-nightly/}.png"
	install -Dm644 "assets/${CLANDRO_PKG_NAME//-nightly/}.desktop" "${CLANDRO_PREFIX}/share/applications/org.wezfurlong.${CLANDRO_PKG_NAME//-nightly/}.desktop"
	install -Dm644 "assets/${CLANDRO_PKG_NAME//-nightly/}.appdata.xml" "${CLANDRO_PREFIX}/share/metainfo/org.wezfurlong.${CLANDRO_PKG_NAME//-nightly/}.appdata.xml"
	install -Dm644 "assets/${CLANDRO_PKG_NAME//-nightly/}-nautilus.py" "${CLANDRO_PREFIX}/share/nautilus-python/extensions/${CLANDRO_PKG_NAME//-nightly/}-nautilus.py"
	install -Dm755 "assets/open-${CLANDRO_PKG_NAME//-nightly/}-here" -t "${CLANDRO_PREFIX}/bin"
	install -Dm644 README.md -t "${CLANDRO_PREFIX}/share/doc/${CLANDRO_PKG_NAME//-nightly/}"
	install -Dm644 assets/shell-completion/bash "${CLANDRO_PREFIX}/share/bash-completion/completions/${CLANDRO_PKG_NAME//-nightly/}"
	install -Dm644 assets/shell-completion/fish "${CLANDRO_PREFIX}/share/fish/vendor_completions.d/${CLANDRO_PKG_NAME//-nightly/}.fish"
	install -Dm644 assets/shell-completion/zsh "${CLANDRO_PREFIX}/share/zsh/site-functions/_${CLANDRO_PKG_NAME//-nightly/}"
	install -Dm644 assets/shell-integration/${CLANDRO_PKG_NAME//-nightly/}.sh -t "${CLANDRO_PREFIX}/etc/profile.d"
}
