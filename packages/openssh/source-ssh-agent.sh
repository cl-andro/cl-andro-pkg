...
if [ -r "${CLANDRO__PREFIX:-"${PREFIX}"}"/etc/ssh/start_agent.sh ]; then
	. "${CLANDRO__PREFIX:-"${PREFIX}"}"/etc/ssh/start_agent.sh
fi

export SSH_AUTH_SOCK="${XDG_RUNTIME_DIR:-"${CLANDRO__PREFIX:-"${PREFIX}"}/var/run"}"/ssh-agent.socket
