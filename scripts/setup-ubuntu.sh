...
# Needed by gobject-introspection (clandro_setup_gir).
...
{
	echo "deb [arch=amd64] http://apt.llvm.org/noble/ llvm-toolchain-noble-${CLANDRO_HOST_LLVM_MAJOR_VERSION} main"
} | $SUDO tee /etc/apt/sources.list.d/apt-llvm-org.list > /dev/null

LLVM_PACKAGES=""

# Needed by rust and other packages.
LLVM_PACKAGES+=" llvm-${CLANDRO_HOST_LLVM_MAJOR_VERSION}-dev"
LLVM_PACKAGES+=" llvm-${CLANDRO_HOST_LLVM_MAJOR_VERSION}-tools"
LLVM_PACKAGES+=" clang-${CLANDRO_HOST_LLVM_MAJOR_VERSION}"
LLVM_PACKAGES+=" lld-${CLANDRO_HOST_LLVM_MAJOR_VERSION}"
...
# Ownership of `CLANDRO__PREFIX` must be fixed before `CLANDRO_APP__DATA_DIR`
# if its under it, otherwise `CLANDRO__ROOTFS` will not have its ownership fixed.
$SUDO mkdir -p "$CLANDRO__PREFIX"
$SUDO chown -R "$(whoami)" "$CLANDRO__PREFIX"
$SUDO mkdir -p "$CLANDRO_APP__DATA_DIR"
$SUDO chown -R "$(whoami)" "${CLANDRO_APP__DATA_DIR%"${CLANDRO_APP__DATA_DIR#/*/}"}" # Get `/path/` from `/path/to/app__data_dir`.

# Initial symbolic link in the symbolic link chain for packages
# that have a build dependency on 'aosp-libs'; see scripts/build/clandro_step_override_config_scripts.sh
# and scripts/build/setup/clandro_setup_proot.sh for more information
$SUDO ln -sf "$CLANDRO_APP__DATA_DIR/aosp" /system
