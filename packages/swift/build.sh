CLANDRO_PKG_HOMEPAGE=https://swift.org/
CLANDRO_PKG_DESCRIPTION="Swift is a high-performance system programming language"
CLANDRO_PKG_LICENSE="Apache-2.0, NCSA"
CLANDRO_PKG_MAINTAINER="@finagolfin"
CLANDRO_PKG_VERSION=6.2.1
SWIFT_RELEASE="RELEASE"
CLANDRO_PKG_SRCURL=https://github.com/swiftlang/swift/archive/refs/tags/swift-$CLANDRO_PKG_VERSION-$SWIFT_RELEASE.tar.gz
CLANDRO_PKG_SHA256=39825af3b1ab523ed4970e1315a14b5379fb5f8046170c27a330a179d982fe2a
CLANDRO_PKG_AUTO_UPDATE=false
CLANDRO_PKG_HOSTBUILD=true
CLANDRO_PKG_DEPENDS="clang, libandroid-execinfo, libandroid-glob, libandroid-posix-semaphore, libandroid-shmem, libandroid-spawn, libandroid-spawn-static, libandroid-sysv-semaphore, libcurl, libuuid, libxml2, libdispatch, llbuild, pkg-config, swift-sdk-${CLANDRO_ARCH/_/-}"
CLANDRO_PKG_BUILD_DEPENDS="rsync"
CLANDRO_PKG_EXCLUDED_ARCHES="i686"
CLANDRO_PKG_NO_STATICSPLIT=true
# Building swift uses CMake, but the standard
# clandro_step_configure_cmake function is not used. Instead, we set
# CLANDRO_PKG_FORCE_CMAKE to make the build system aware that CMake is
# needed.
CLANDRO_PKG_FORCE_CMAKE=true
CLANDRO_PKG_CMAKE_BUILD=Ninja
...
SWIFT_TOOLCHAIN_FLAGS="-RA --llvm-targets-to-build='X86;ARM;AArch64' -j $CLANDRO_PKG_MAKE_PROCESSES --install-prefix=$CLANDRO_PREFIX"
SWIFT_PATH_FLAGS="--build-subdir=. --install-destdir=/"
SWIFT_BUILD_FLAGS="$SWIFT_TOOLCHAIN_FLAGS $SWIFT_PATH_FLAGS"

SWIFT_ARCH=$CLANDRO_ARCH
...
clandro_step_post_get_source() {
	...
	for library in "${!library_checksums[@]}"; do \
		...
		if [ "$library" = "swift-argument-parser" ]; then
			...
			TAR_NAME=swift-$CLANDRO_PKG_VERSION-$SWIFT_RELEASE
		fi

		clandro_download \
			https://github.com/$GH_ORG/$library/archive/refs/tags/$TAR_NAME.tar.gz \
			$CLANDRO_PKG_CACHEDIR/$library-$SRC_VERSION.tar.gz \
			${library_checksums[$library]}
		tar xf $CLANDRO_PKG_CACHEDIR/$library-$SRC_VERSION.tar.gz
...
clandro_step_host_build() {
	if [ "$CLANDRO_ON_DEVICE_BUILD" = "false" ]; then
		...
		if [ -z "$CLANG" ]; then
			CLANG="$CLANDRO_HOST_LLVM_BASE_DIR/bin/clang"
			CLANGXX="$CLANDRO_HOST_LLVM_BASE_DIR/bin/clang++"
		fi

		# Natively compile llvm-tblgen and some other files needed later.
		SWIFT_BUILD_ROOT=$CLANDRO_PKG_HOSTBUILD_DIR $CLANDRO_PKG_SRCDIR/swift/utils/build-script \
		-R --no-assertions -j $CLANDRO_PKG_MAKE_PROCESSES $SWIFT_PATH_FLAGS \
...
clandro_step_make() {
	...
	echo "then you should try setting CLANDRO_PKG_MAKE_PROCESSES=4 or a lower value."

	if [ "$CLANDRO_ON_DEVICE_BUILD" = "false" ]; then
		clandro_setup_swift
		ln -sf $CLANDRO_PKG_HOSTBUILD_DIR/llvm-linux-x86_64 $CLANDRO_PKG_BUILDDIR/llvm-linux-x86_64
		for header in execinfo.h glob.h iconv.h spawn.h sys/sem.h sys/shm.h; do
			ln -sf $CLANDRO_PREFIX/include/$header $CLANDRO_STANDALONE_TOOLCHAIN/sysroot/usr/include/$header
		done
		unset ANDROID_NDK_ROOT

		SWIFT_BUILD_FLAGS="$SWIFT_BUILD_FLAGS --android
		--android-ndk $CLANDRO_STANDALONE_TOOLCHAIN --android-arch $SWIFT_ARCH
		--build-toolchain-only --skip-local-build --skip-local-host-install
		--cross-compile-hosts=android-$SWIFT_ARCH
		--cross-compile-deps-path=$(dirname $CLANDRO_PREFIX)
		--native-swift-tools-path=$SWIFT_BINDIR
		--native-clang-tools-path=$SWIFT_BINDIR
		--cross-compile-append-host-target-to-destdir=False"
	fi

	SWIFT_BUILD_ROOT=$CLANDRO_PKG_BUILDDIR $CLANDRO_PKG_SRCDIR/swift/utils/build-script \
	$SWIFT_BUILD_FLAGS --xctest --swift-testing -b -p --swift-driver --sourcekit-lsp \
	--android-api-level $CLANDRO_PKG_API_LEVEL --build-swift-static-stdlib \
	...
	rm $CLANDRO_STANDALONE_TOOLCHAIN/sysroot/usr/include/{execinfo.h,glob.h,iconv.h,spawn.h,sys/sem.h,sys/shm.h}
	rm $CLANDRO_STANDALONE_TOOLCHAIN/sysroot/usr/lib/swift
}

clandro_step_make_install() {
	rm -rf $CLANDRO_PREFIX/lib/swift{,_static}/{Block,os}
	rm $CLANDRO_PREFIX/lib/swift{,_static}/dispatch/*.h
	rm $CLANDRO_PREFIX/lib/swift/android/lib{dispatch,BlocksRuntime}.so
	mv $CLANDRO_PREFIX/lib/swift/android/lib[^_]*.so $CLANDRO_PREFIX/opt/ndk-multilib/$CLANDRO_ARCH-linux-android*/lib
	mv $CLANDRO_PREFIX/lib/swift/android/lib_{Testing_,}Foundation*.so $CLANDRO_PREFIX/opt/ndk-multilib/$CLANDRO_ARCH-linux-android*/lib
	mv $CLANDRO_PREFIX/lib/swift/android/lib*.a $CLANDRO_PREFIX/lib/swift/android/$SWIFT_ARCH
	mv $CLANDRO_PREFIX/lib/swift_static/android/lib*.a $CLANDRO_PREFIX/lib/swift_static/android/$SWIFT_ARCH

	if [ "$CLANDRO_ON_DEVICE_BUILD" = "false" ]; then
		rm $CLANDRO_PREFIX/swiftpm-android-$SWIFT_ARCH.json
	fi
}
