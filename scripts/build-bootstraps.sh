...
# Description:   A script to build bootstrap archives for the clandro-app
#                from local package sources instead of debs published in
#                apt repo like done by generate-bootstrap.sh. It allows
#                bootstrap archives to be easily built for clandro
...
export CLANDRO_SCRIPTDIR=$(realpath "$(dirname "$(realpath "$0")")/../")
: "${CLANDRO_TOPDIR:="$HOME/.clandro-build"}"
. "${CLANDRO_SCRIPTDIR}"/scripts/properties.sh
. "${CLANDRO_SCRIPTDIR}"/scripts/build/clandro_step_handle_buildarch.sh
...
# supported by Clandro application.
# Override with option '--architectures'.
CLANDRO_DEFAULT_ARCHITECTURES=("aarch64" "arm" "i686" "x86_64")
CLANDRO_ARCHITECTURES=("${CLANDRO_DEFAULT_ARCHITECTURES[@]}")

CLANDRO_PACKAGES_DIRECTORY="/home/builder/clandro-packages"
CLANDRO_BUILT_DEBS_DIRECTORY="$CLANDRO_PACKAGES_DIRECTORY/output"
...
build_package() {
	...
	cd "$CLANDRO_PACKAGES_DIRECTORY"
	echo $'\n\n\n'"[*] Building '$package_name'..."
	exec 99>&1
	build_output="$("$CLANDRO_PACKAGES_DIRECTORY"/build-package.sh "${BUILD_PACKAGE_OPTIONS[@]}" -a "$CLANDRO_ARCH" "$package_name" 2>&1 | tee >(cat - >&99); exit ${PIPESTATUS[0]})";
	return_value=$?
	echo "[*] Building '$package_name' exited with exit code $return_value"
	exec 99>&-
	if [ $return_value -ne 0 ]; then
		echo "Failed to build package '$package_name' for arch '$CLANDRO_ARCH'" 1>&2
...
extract_debs() {
	...
	cd "$CLANDRO_BUILT_DEBS_DIRECTORY"
	...
	for deb in *.deb; do
		...
		echo "[*] Extracting '$deb'..."
		(cd "$package_tmpdir"
			ar x "$CLANDRO_BUILT_DEBS_DIRECTORY/$deb"
			...
			if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE}; then
				...
				tar tf "$data_archive" | sed -E -e 's@^\./@/@' -e 's@^/$@/.@' -e 's@^([^./])@/\1@' > "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/info/${current_package_name}.list"

				# Generate checksums (md5).
				tar xf "$data_archive"
				find data -type f -print0 | xargs -0 -r md5sum | sed 's@^\.$@@g' > "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/info/${current_package_name}.md5sums"
				...
				tar xf "$control_archive"
				{
					...
					echo
				} >> "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/status"

				# Additional data: conffiles & scripts
				for file in conffiles postinst postrm preinst prerm; do
					if [ -f "${PWD}/${file}" ]; then
						cp "$file" "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/info/${current_package_name}.${file}"
					fi
				done
			fi
		)
...
# Add clandro bootstrap second stage files
add_clandro_bootstrap_second_stage_files() {

	local package_arch="$1"

	echo $'\n\n\n'"[*] Adding clandro bootstrap second stage files..."

	mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR}"
	sed -e "s|@CLANDRO_PREFIX@|${CLANDRO_PREFIX}|g" \
		-e "s|@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR@|${CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR}|g" \
		-e "s|@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@|${CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE}|g" \
		-e "s|@CLANDRO_PACKAGE_MANAGER@|${CLANDRO_PACKAGE_MANAGER}|g" \
		-e "s|@CLANDRO_PACKAGE_ARCH@|${package_arch}|g" \
		-e "s|@CLANDRO_APP__NAME@|${CLANDRO_APP__NAME}|g" \
		-e "s|@CLANDRO_ENV__S_CLANDRO@|${CLANDRO_ENV__S_CLANDRO}|g" \
		"$CLANDRO_SCRIPTDIR/scripts/bootstrap/$CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE" \
		> "${BOOTSTRAP_ROOTFS}/${CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR}/$CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE"
	chmod 700 "${BOOTSTRAP_ROOTFS}/${CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR}/$CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE"

	# TODO: Remove it when Termux app supports `pacman` bootstraps installation.
	sed -e "s|@CLANDRO_PREFIX@|${CLANDRO_PREFIX}|g" \
		-e "s|@CLANDRO__PREFIX__PROFILE_D_DIR@|${CLANDRO__PREFIX__PROFILE_D_DIR}|g" \
		-e "s|@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR@|${CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR}|g" \
		-e "s|@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@|${CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE}|g" \
		"$CLANDRO_SCRIPTDIR/scripts/bootstrap/01-clandro-bootstrap-second-stage-fallback.sh" \
		> "${BOOTSTRAP_ROOTFS}/${CLANDRO__PREFIX__PROFILE_D_DIR}/01-clandro-bootstrap-second-stage-fallback.sh"
	chmod 600 "${BOOTSTRAP_ROOTFS}/${CLANDRO__PREFIX__PROFILE_D_DIR}/01-clandro-bootstrap-second-stage-fallback.sh"
...
create_bootstrap_archive() {
	...
	(cd "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}"
	...
	mv -f "${BOOTSTRAP_TMPDIR}/bootstrap-${1}.zip" "$CLANDRO_PACKAGES_DIRECTORY/"
...
build_bootstrap_trap() {
	...
	[ -h "$CLANDRO_BUILT_PACKAGES_DIRECTORY" ] && rm -f "$CLANDRO_BUILT_PACKAGES_DIRECTORY"
...
clandro-app from local package sources instead of debs published in
apt repo like done by generate-bootstrap.sh. It allows bootstrap archives
to be easily built for clandro apps without having to publish
an apt repo first.
...
CLANDRO_APP_PACKAGE in 'scrips/properties.sh'. It defaults to 'com.zk.clandro'.
...
echo "CLANDRO_PREFIX: \"${CLANDRO_PREFIX[*]}\""
echo "CLANDRO_ARCHITECTURES: \"${CLANDRO_ARCHITECTURES[*]}\""
...
main() {
	...
	while (($# > 0)); do
		case "$1" in
			...
			--architectures)
				if [ $# -gt 1 ] && [ -n "$2" ] && [[ $2 != -* ]]; then
					CLANDRO_ARCHITECTURES=()
					for arch in $(echo "$2" | tr ',' ' '); do
						CLANDRO_ARCHITECTURES+=("$arch")
					done
					unset arch
					shift 1
				else
	...
	for CLANDRO_ARCH in "${CLANDRO_ARCHITECTURES[@]}"; do
		if [[ " ${CLANDRO_DEFAULT_ARCHITECTURES[*]} " != *" $CLANDRO_ARCH "* ]]; then
			echo "Unsupported architecture '$CLANDRO_ARCH' for in architectures list: '${CLANDRO_ARCHITECTURES[*]}'" 1>&2
			echo "Supported architectures: '${CLANDRO_DEFAULT_ARCHITECTURES[*]}'" 1>&2
			return 1
		fi
	done
	...
	for CLANDRO_ARCH in "${CLANDRO_ARCHITECTURES[@]}"; do
		...
		if [[ $FORCE_BUILD_PACKAGES == "1" ]]; then
			rm -f "$CLANDRO_BUILT_PACKAGES_DIRECTORY_FOR_ARCH"/*
			rm -f "$CLANDRO_BUILT_DEBS_DIRECTORY"/*
		fi

		BOOTSTRAP_ROOTFS="$BOOTSTRAP_TMPDIR/rootfs-${CLANDRO_ARCH}"
		BOOTSTRAP_PKGDIR="$BOOTSTRAP_TMPDIR/packages-${CLANDRO_ARCH}"

		# Create initial directories for $CLANDRO_PREFIX
		if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE}; then
			mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/etc/apt/apt.conf.d"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/etc/apt/preferences.d"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/info"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/triggers"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/updates"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/log/apt"
			touch "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/available"
			touch "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/status"
		fi
		mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/tmp"
		...
		# Core utilities.
		PACKAGES+=("bash") # Used by `clandro-bootstrap-second-stage.sh`
		PACKAGES+=("bzip2")
		if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE}; then
		...
		PACKAGES+=("sed")
		PACKAGES+=("tar")
		PACKAGES+=("clandro-core")
		PACKAGES+=("clandro-exec")
		PACKAGES+=("clandro-keyring")
		PACKAGES+=("clandro-tools")
		PACKAGES+=("util-linux")
		...
		for package_name in "${PACKAGES[@]}"; do
			set +e
			build_package "$CLANDRO_ARCH" "$package_name" || return $?
			set -e
		done

		# Extract all debs.
		extract_debs "$CLANDRO_ARCH" || return $?

		# Add clandro bootstrap second stage files
		add_clandro_bootstrap_second_stage_files "$package_arch"

		# Create bootstrap archive.
		create_bootstrap_archive "$CLANDRO_ARCH" || return $?
