clandro_setup_nodejs() {
	export NODE_OPTIONS=""
	# This should not be needed when we update nodejs version to v26
	# This is the default from v25.2.0 onwards, and first LTS having it will be v26
	# Ref: https://github.com/nodejs/node/commit/506b79e888
	NODE_OPTIONS+=" --network-family-autoselection-attempt-timeout=500"
	# Use LTS version for now
	local NODEJS_VERSION=22.22.2
	local NODEJS_FOLDER

	if [ "${CLANDRO_PACKAGES_OFFLINE-false}" = "true" ]; then
		NODEJS_FOLDER=${CLANDRO_SCRIPTDIR}/build-tools/nodejs-${NODEJS_VERSION}
	else
		NODEJS_FOLDER=${CLANDRO_COMMON_CACHEDIR}/nodejs-$NODEJS_VERSION
	fi

	if [ "$CLANDRO_ON_DEVICE_BUILD" = "false" ]; then
		if [ ! -x "$NODEJS_FOLDER/bin/node" ]; then
			mkdir -p "$NODEJS_FOLDER"
			local NODEJS_TAR_FILE=$CLANDRO_PKG_TMPDIR/nodejs-$NODEJS_VERSION.tar.xz
			clandro_download https://nodejs.org/dist/v${NODEJS_VERSION}/node-v${NODEJS_VERSION}-linux-x64.tar.xz \
				"$NODEJS_TAR_FILE" \
				88fd1ce767091fd8d4a99fdb2356e98c819f93f3b1f8663853a2dee9b438068a
			tar -xf "$NODEJS_TAR_FILE" -C "$NODEJS_FOLDER" --strip-components=1
		fi
		export PATH=$NODEJS_FOLDER/bin:$PATH
	else
		local NODEJS_PKG_VERSION=$(bash -c ". $CLANDRO_SCRIPTDIR/packages/nodejs/build.sh; echo \$CLANDRO_PKG_VERSION")
		if ([ ! -e "$CLANDRO_BUILT_PACKAGES_DIRECTORY/nodejs" ] ||
		    [ "$(cat "$CLANDRO_BUILT_PACKAGES_DIRECTORY/nodejs")" != "$NODEJS_PKG_VERSION" ]) &&
		   ([[ "$CLANDRO_APP_PACKAGE_MANAGER" = "apt" && "$(dpkg-query -W -f '${db:Status-Status}\n' nodejs 2>/dev/null)" != "installed" ]] ||
		    [[ "$CLANDRO_APP_PACKAGE_MANAGER" = "pacman" && ! "$(pacman -Q nodejs 2>/dev/null)" ]]); then
			echo "Package 'nodejs' is not installed."
			echo "You can install it with"
			echo
			echo "  pkg install nodejs"
			echo
			echo "  pacman -S nodejs"
			echo
			echo "or build it from source with"
			echo
			echo "  ./build-package.sh nodejs"
			echo
			exit 1
		fi
	fi
}
