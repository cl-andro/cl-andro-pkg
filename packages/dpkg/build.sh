CLANDRO_PKG_HOMEPAGE=https://packages.debian.org/dpkg
CLANDRO_PKG_DESCRIPTION="Debian package management system"
CLANDRO_PKG_LICENSE="GPL-2.0"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION="1.22.6"
CLANDRO_PKG_REVISION=5
# old tarball are removed in https://mirrors.kernel.org/debian/pool/main/d/dpkg/dpkg_${CLANDRO_PKG_VERSION}.tar.xz
CLANDRO_PKG_SRCURL=git+https://salsa.debian.org/dpkg-team/dpkg.git
CLANDRO_PKG_GIT_BRANCH="${CLANDRO_PKG_VERSION}"
CLANDRO_PKG_AUTO_UPDATE=false
CLANDRO_PKG_DEPENDS="bzip2, coreutils, diffutils, gzip, less, libbz2, liblzma, libmd, tar, xz-utils, zlib, zstd"
CLANDRO_PKG_ANTI_BUILD_DEPENDS="clang"
CLANDRO_PKG_BREAKS="dpkg-dev"
CLANDRO_PKG_REPLACES="dpkg-dev"
...
clandro_step_pre_configure() {
	(
		...
		./autogen && \
		patch -p1 -i "${CLANDRO_PKG_BUILDER_DIR}"/configure.diff
	)
	export TAR=tar # To make sure dpkg tries to use "tar" instead of e.g. "gnutar" (which happens when building on OS X)
	perl -p -i -e "s/CLANDRO_ARCH/$CLANDRO_ARCH/" $CLANDRO_PKG_SRCDIR/configure
	sed -i 's/$req_vars = \$arch_vars.$varname./if ($varname eq "DEB_HOST_ARCH_CPU" or $varname eq "DEB_HOST_ARCH"){ print("'$CLANDRO_ARCH'");exit; }; $req_vars = $arch_vars{$varname}/' scripts/dpkg-architecture.pl
}

clandro_step_post_massage() {
	mkdir -p "${CLANDRO_PKG_MASSAGEDIR}/${CLANDRO_PREFIX}/var/lib/dpkg/alternatives"
	mkdir -p "${CLANDRO_PKG_MASSAGEDIR}/${CLANDRO_PREFIX}/var/lib/dpkg/info"
	mkdir -p "${CLANDRO_PKG_MASSAGEDIR}/${CLANDRO_PREFIX}/var/lib/dpkg/triggers"
	mkdir -p "${CLANDRO_PKG_MASSAGEDIR}/${CLANDRO_PREFIX}/var/lib/dpkg/updates"
}
