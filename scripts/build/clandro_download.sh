...
clandro_download() {
	if [[ $# != 2 ]] && [[ $# != 3 ]]; then
		echo "clandro_download(): Invalid arguments - expected <URL> <DESTINATION> [<CHECKSUM>]" 1>&2
		return 1
	fi
	local URL="$1"
	local DESTINATION="$2"
	...
	TMPFILE=$(mktemp "$CLANDRO_PKG_TMPDIR/download.${CLANDRO_PKG_NAME-unnamed}.XXXXXXXXX")
