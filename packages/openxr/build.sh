CLANDRO_PKG_HOMEPAGE=https://www.khronos.org/openxr/
CLANDRO_PKG_DESCRIPTION="Open standard that provides a common set of APIs for developing XR applications"
CLANDRO_PKG_LICENSE="Apache-2.0"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION="1.1.59"
CLANDRO_PKG_SRCURL="https://github.com/KhronosGroup/OpenXR-SDK-Source/releases/download/release-$CLANDRO_PKG_VERSION/OpenXR-SDK-Source-release-$CLANDRO_PKG_VERSION.tar.gz"
CLANDRO_PKG_SHA256=3b241897f93e2cf7e833d27f8ec3c3433cf5cf064e13724af026984acede02e7
# configuration error if jsoncpp and jsoncpp-static are not both installed simultaneously
CLANDRO_PKG_DEPENDS="jsoncpp, libc++, libx11, vulkan-icd, libxrandr, libxxf86vm, opengl"
CLANDRO_PKG_AUTO_UPDATE=true
CLANDRO_PKG_UPDATE_VERSION_REGEXP="\d+.\d+.\d+"
CLANDRO_PKG_BUILD_DEPENDS="jsoncpp-static, vulkan-headers"
CLANDRO_PKG_EXTRA_CONFIGURE_ARGS="
-DCMAKE_INSTALL_LIBDIR=$CLANDRO__PREFIX__LIB_SUBDIR
-DCMAKE_INSTALL_INCLUDEDIR=$CLANDRO__PREFIX__INCLUDE_SUBDIR
-DCMAKE_SYSTEM_NAME=Linux
-DPRESENTATION_BACKEND=xlib
-DDYNAMIC_LOADER=ON
"

clandro_step_pre_configure() {
	# This software has a SurfaceFlinger(ANativeWindow[For building an APK]) configuration
	# aggressively connected to its __ANDROID__ blocks, so use the same 'treat Android as Linux'
	# strategy here that is used in the sdl2 package.
	find "$CLANDRO_PKG_SRCDIR" -type f | \
		xargs -n 1 sed -i \
		-e 's/\([^A-Za-z0-9_]__ANDROID\)\(__[^A-Za-z0-9_]\)/\1_NO_TERMUX\2/g' \
		-e 's/\([^A-Za-z0-9_]__ANDROID\)__$/\1_NO_TERMUX__/g'
}
