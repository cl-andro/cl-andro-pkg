CLANDRO_PKG_HOMEPAGE=https://github.com/lbeckman314/mdbook-latex
CLANDRO_PKG_DESCRIPTION="An mdbook backend for generating LaTeX and PDF documents"
CLANDRO_PKG_LICENSE="MPL-2.0"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION=0.1.42
CLANDRO_PKG_SRCURL=https://github.com/lbeckman314/mdbook-latex/archive/refs/tags/v${CLANDRO_PKG_VERSION}.tar.gz
CLANDRO_PKG_SHA256=4b480a79e491a49653104de51d3ee409929093ffef04b2b2c707f09e7ce2e1f8
CLANDRO_PKG_DEPENDS="fontconfig, freetype, harfbuzz, libexpat, libgraphite, libicu, libpng, openssl-1.1, zlib"
CLANDRO_PKG_BUILD_IN_SRC=true

clandro_step_pre_configure() {
	# openssl-sys supports OpenSSL 3 in >= 0.9.69
	export OPENSSL_INCLUDE_DIR=$CLANDRO_PREFIX/include/openssl-1.1
	export OPENSSL_LIB_DIR=$CLANDRO_PREFIX/lib/openssl-1.1
	CFLAGS="-I$CLANDRO_PREFIX/include/openssl-1.1 $CFLAGS"
	CPPFLAGS="-I$CLANDRO_PREFIX/include/openssl-1.1 $CPPFLAGS"
	CXXFLAGS="-I$CLANDRO_PREFIX/include/openssl-1.1 $CXXFLAGS"
	LDFLAGS="-L$CLANDRO_PREFIX/lib/openssl-1.1 -Wl,-rpath=$CLANDRO_PREFIX/lib/openssl-1.1 $LDFLAGS"
	local env_host=$(printf $CARGO_TARGET_NAME | tr a-z A-Z | sed s/-/_/g)
	export CARGO_TARGET_${env_host}_RUSTFLAGS+=" -C link-arg=-Wl,-rpath=$CLANDRO_PREFIX/lib/openssl-1.1"

	clandro_setup_rust

	: "${CARGO_HOME:=$HOME/.cargo}"
	export CARGO_HOME

	cargo fetch --target "${CARGO_TARGET_NAME}"

	local c
	for c in {expat,servo-{fontconfig,freetype}}-sys; do
		local p=$CLANDRO_PKG_BUILDER_DIR/${c}-build.rs.diff
		local d
		for d in $CARGO_HOME/registry/src/github.com-*/${c}-*; do
			patch --silent -p1 -d ${d} < ${p} || :
		done
	done

	local d
	for d in $CARGO_HOME/registry/src/github.com-*/usvg-*; do
		sed 's|@CLANDRO_PREFIX@|'"${CLANDRO_PREFIX}"'|g' \
			$CLANDRO_PKG_BUILDER_DIR/usvg-src-fontdb.rs.diff \
			| patch --silent -p1 -d ${d} || :
	done

	local _patch=$CLANDRO_PKG_BUILDER_DIR/mdbook-src-renderer-html_handlebars-helpers-navigation.rs.diff
	for d in $CARGO_HOME/registry/src/github.com-*/mdbook-*; do
		patch --silent -p1 -d ${d} < ${_patch} || :
	done
}

clandro_step_make() {
	cargo build --jobs $CLANDRO_PKG_MAKE_PROCESSES --target $CARGO_TARGET_NAME --release
}

clandro_step_make_install() {
	install -Dm700 -t $CLANDRO_PREFIX/bin target/${CARGO_TARGET_NAME}/release/mdbook-latex
}
