CLANDRO_PKG_HOMEPAGE=https://github.com/libsdl-org/SDL_mixer
CLANDRO_PKG_DESCRIPTION="A simple multi-channel audio mixer library (version 3)"
CLANDRO_PKG_LICENSE="ZLIB"
CLANDRO_PKG_LICENSE_FILE="LICENSE.txt"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION="3.2.0"
CLANDRO_PKG_REVISION=1
CLANDRO_PKG_SRCURL=https://github.com/libsdl-org/SDL_mixer/releases/download/release-${CLANDRO_PKG_VERSION}/SDL3_mixer-${CLANDRO_PKG_VERSION}.tar.gz
CLANDRO_PKG_SHA256=1f86fae7226d58f2ad210ca4d9e06488db722230032803423d83bad6d35fc395
CLANDRO_PKG_AUTO_UPDATE=true
CLANDRO_PKG_DEPENDS="libflac, libxmp, fluidsynth, libogg, libvorbis, libmodplug, opusfile, libwavpack, libmpg123, sdl3"

CLANDRO_PKG_EXTRA_CONFIGURE_ARGS="
-DSDLMIXER_VENDORED=OFF
-DSDLMIXER_VORBIS_STB=OFF
-DSDLMIXER_VORBIS_VORBISFILE=ON
-DSDLMIXER_FLAC_DRFLAC=OFF
-DSDLMIXER_FLAC_LIBFLAC=ON
-DSDLMIXER_MP3_DRMP3=OFF
-DSDLMIXER_MP3_MPG123=ON
-DSDLMIXER_OGG=ON
-DSDLMIXER_OPUS=ON
-DSDLMIXER_GME=ON
-DSDLMIXER_MIDI_FLUIDSYNTH=ON
-DSDLMIXER_WAVPACK=ON
-DSDLMIXER_MOD_XMP=ON
-DSDLMIXER_OGG_SHARED=OFF
-DSDLMIXER_OPUS_SHARED=OFF
-DSDLMIXER_GME_SHARED=OFF
-DSDLMIXER_WAVPACK_SHARED=OFF
-DSDLMIXER_FLAC_LIBFLAC_SHARED=OFF
-DSDLMIXER_VORBIS_VORBISFILE_SHARED=OFF
-DSDLMIXER_MP3_MPG123_SHARED=OFF
-DSDLMIXER_MIDI_FLUIDSYNTH_SHARED=OFF
"
