...
clandro_setup_dotnet() {
	...
	# This has no effect on Clandro dotnet (telemetry is already disabled)
	...
	if [[ -z "${CLANDRO_DOTNET_VERSION-}" ]]; then
		# LTS version
	...
	if [[ "${CLANDRO_ON_DEVICE_BUILD}" == "true" ]]; then
		...
		if [[ -n "${CLANDRO_DOTNET_VERSION-}" ]] && [[ "${CLANDRO_DOTNET_VERSION-}" != "${DOTNET_VERSION//.*}"* ]]; then
			cat <<- EOL >&2
			WARN: Mismatch dotnet version!
			CLANDRO_DOTNET_VERSION = ${CLANDRO_DOTNET_VERSION}
	...
	curl https://raw.githubusercontent.com/dotnet/install-scripts/refs/heads/main/src/dotnet-install.sh -sSfo "${CLANDRO_PKG_TMPDIR}"/dotnet-install.sh
	bash "${CLANDRO_PKG_TMPDIR}"/dotnet-install.sh --channel "${CLANDRO_DOTNET_VERSION}"
