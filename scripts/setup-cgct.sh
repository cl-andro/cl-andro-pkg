#!/usr/bin/env bash
# setup CGCT - Cross Gnu Compilers for Clandro
# compile glibc-based binaries for Clandro

. $(dirname "$(realpath "$0")")/properties.sh
. $(dirname "$(realpath "$0")")/build/clandro_download.sh
...
declare -A CGCT=(
	["cbt"]="2.45.1-0" # Cross Binutils for Clandro
	["cgt"]="15.2.0-0" # Cross GCCs for Clandro
	["glibc-cgct"]="2.42-0" # Glibc for CGCT
 	["cgct-headers"]="6.18.6-0" # Headers for CGCT
)

: "${CLANDRO_PKG_TMPDIR:="/tmp"}"
TMPDIR_CGCT="${CLANDRO_PKG_TMPDIR}/cgct"
...
for pkgname in ${!CGCT[@]}; do
	...
	if [ ! -f "${TMPDIR_CGCT}/${filename}" ]; then
		clandro_download "${REPO_URL}/${filename}" \
...
if [ ! -f "${CGCT_DIR}/lib/libgcc_s.so" ]; then
	...
	clandro_download "https://archive.archlinux.org/packages/g/gcc-libs/gcc-libs-15.1.1+r7+gf36ec88aa85a-1-x86_64.pkg.tar.zst" \
