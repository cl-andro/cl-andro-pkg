...
clandro_pkg_is_update_needed() {
	# USAGE: clandro_pkg_is_update_needed <current-version> <latest-version>
	if [[ -z "$1" ]] || [[ -z "$2" ]]; then
		clandro_error_exit "${BASH_SOURCE[0]}: at least 2 arguments expected"
	fi
	...
	if ! dpkg --validate-version "${LATEST_VERSION}" &> /dev/null; then
		echo "::warning::${CLANDRO_PKG_NAME:-}: $(dpkg --validate-version "${LATEST_VERSION}" &> /dev/stdout)" >&2
		return 1
	fi
	...
	case "$DPKG_EXIT_CODE" in
		...
		*) clandro_error_exit "Bad 'dpkg --compare-versions' exit code: $DPKG_EXIT_CODE - bad version numbers?" ;;
	esac
}
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
	...
	declare -f clandro_error_exit >/dev/null ||
		. "$(dirname "$(realpath "${BASH_SOURCE[0]}")")/../../build/clandro_error_exit.sh" # realpath is used to resolve symlinks.
	...
	if [[ -n "${version_regexp}" ]]; then
		...
		if [[ -z "${first_version}" ]] || [[ -z "${second_version}" ]]; then
			clandro_error_exit "Unable to parse version numbers using regexp '${version_regexp}'"
		fi
	fi
	if [[ "${first_version}" == "${second_version}" ]]; then
		echo "${first_version} = ${second_version}"
	else
		if clandro_pkg_is_update_needed "${first_version}" "${second_version}"; then
			echo "${first_version} < ${second_version}"
		elif clandro_pkg_is_update_needed "${second_version}" "${first_version}"; then
			echo "${first_version} > ${second_version}"
		else
			echo "${first_version} = ${second_version}"
		fi
