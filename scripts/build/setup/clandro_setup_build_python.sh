...
clandro_setup_build_python() {
	if [[ "$CLANDRO_ON_DEVICE_BUILD" == "true" ]]; then
		...
		[[ "$CLANDRO_APP_PACKAGE_MANAGER" = "pacman" && ! "$(pacman -Q python 2>/dev/null)" ]]; then
		...
		_PYTHON_VERSION="$(. "$CLANDRO_SCRIPTDIR/packages/python/build.sh"; echo "$CLANDRO_PKG_VERSION")"
		_PYTHON_SRCURL="$(. "$CLANDRO_SCRIPTDIR/packages/python/build.sh"; echo "$CLANDRO_PKG_SRCURL")"
		_PYTHON_SHA256="$(. "$CLANDRO_SCRIPTDIR/packages/python/build.sh"; echo "$CLANDRO_PKG_SHA256")"
		if [[ "${CLANDRO_PACKAGES_OFFLINE-false}" = "true" ]]; then
			_PYTHON_FOLDER=${CLANDRO_SCRIPTDIR}/build-tools/python-${_PYTHON_VERSION}
		else
			_PYTHON_FOLDER=${CLANDRO_COMMON_CACHEDIR}/python-${_PYTHON_VERSION}
		fi
		export CLANDRO_BUILD_PYTHON_DIR=$_PYTHON_FOLDER
		...
		if [[ ! -d "$_PYTHON_FOLDER" ]]; then
			...
			for f in "$CLANDRO_SCRIPTDIR"/packages/python/0008-fix-ctypes-util-find_library.patch; do
				echo "[${FUNCNAME[0]}]: Applying $(basename "$f")"
				cat "$f" | sed -e "s|@@CLANDRO_PKG_API_LEVEL@@|${CLANDRO_PKG_API_LEVEL}|g" | patch --silent -p1
			done
			...
			cd host-build/ && \
			env -i \
				CC="clang-${CLANDRO_HOST_LLVM_MAJOR_VERSION} -fuse-ld=lld" \
				CXX="clang++-${CLANDRO_HOST_LLVM_MAJOR_VERSION} -fuse-ld=lld" \
