CLANDRO_PKG_HOMEPAGE=https://emscripten.org
CLANDRO_PKG_DESCRIPTION="Emscripten: An LLVM-to-WebAssembly Compiler"
CLANDRO_PKG_LICENSE="MIT"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION="5.0.7"
CLANDRO_PKG_SRCURL=git+https://github.com/emscripten-core/emscripten
CLANDRO_PKG_GIT_BRANCH=${CLANDRO_PKG_VERSION}
CLANDRO_PKG_DEPENDS="nodejs-lts | nodejs, python"
CLANDRO_PKG_ANTI_BUILD_DEPENDS="nodejs, nodejs-lts, python"
...
# refer clandro_step_post_get_source and clandro_step_post_massage
...
-DCMAKE_INSTALL_PREFIX=${CLANDRO_PREFIX}/opt/emscripten-llvm
...
-DCMAKE_INSTALL_PREFIX=${CLANDRO_PREFIX}/opt/emscripten-binaryen
-DBUILD_TESTS=OFF
-DBYN_ENABLE_LTO=ON
"

# based on scripts/updates/internal/clandro_github_auto_update.sh
...
clandro_step_post_get_source() {
	# for comparing files in clandro_step_post_massage
	if [[ ! -f "${CLANDRO_PKG_CACHEDIR}/emsdk-fetched" || $(cat "${CLANDRO_PKG_CACHEDIR}/emsdk-fetched") != "$CLANDRO_PKG_VERSION" ]]; then
		pushd "${CLANDRO_PKG_CACHEDIR}"
		rm -fr emsdk
		git clone https://github.com/emscripten-core/emsdk --depth=1
		cd emsdk
		./emsdk install latest
		echo "$CLANDRO_PKG_VERSION" > "${CLANDRO_PKG_CACHEDIR}"/emsdk-fetched
		popd
	fi

	clandro_download \
		"https://github.com/llvm/llvm-project/archive/${_LLVM_COMMIT}.tar.gz" \
		"${CLANDRO_PKG_CACHEDIR}/llvm.tar.gz" \
		"${_LLVM_TGZ_SHA256}"
	clandro_download \
		"https://github.com/WebAssembly/binaryen/archive/${_BINARYEN_COMMIT}.tar.gz" \
		"${CLANDRO_PKG_CACHEDIR}/binaryen.tar.gz" \
		"${_BINARYEN_TGZ_SHA256}"
	rm -rf "${CLANDRO_PKG_CACHEDIR}/llvm-project-${_LLVM_COMMIT}"
	tar -xf "${CLANDRO_PKG_CACHEDIR}/llvm.tar.gz" -C "${CLANDRO_PKG_CACHEDIR}"
	rm -rf "${CLANDRO_PKG_CACHEDIR}/binaryen-${_BINARYEN_COMMIT}"
	tar -xf "${CLANDRO_PKG_CACHEDIR}/binaryen.tar.gz" -C "${CLANDRO_PKG_CACHEDIR}"

	local llvm_patches=$(find "${CLANDRO_PKG_BUILDER_DIR}" -mindepth 1 -maxdepth 1 -type f -name 'llvm-project-*.diff')
	if [[ -n "${llvm_patches}" ]]; then
		pushd "${CLANDRO_PKG_CACHEDIR}/llvm-project-${_LLVM_COMMIT}"
		...
		if [[ -n "${llvm_patches_rej}" ]]; then
			...
			clandro_error_exit "Patch failed! Please check patch errors above!"
		fi
		popd
	fi

	local binaryen_patches=$(find "${CLANDRO_PKG_BUILDER_DIR}" -mindepth 1 -maxdepth 1 -type f -name 'binaryen-*.diff')
	if [[ -n "${binaryen_patches}" ]]; then
		pushd "${CLANDRO_PKG_CACHEDIR}/binaryen-${_BINARYEN_COMMIT}"
...
clandro_step_host_build() {
	...
	cmake \
		-G Ninja \
		-S "${CLANDRO_PKG_CACHEDIR}/llvm-project-${_LLVM_COMMIT}/llvm" \
...
clandro_step_pre_configure() {
	...
	CLANDRO_PKG_DEPENDS+=", emscripten-binaryen, emscripten-llvm"
...
clandro_step_make_install() {
	...
	*) clandro_error_exit "Invalid arch: ${CLANDRO_ARCH}" ;;
	...
	cmake \
		-G Ninja \
		-S "${CLANDRO_PKG_CACHEDIR}/llvm-project-${_LLVM_COMMIT}/llvm" \
		-B "${CLANDRO_PKG_BUILDDIR}/build-llvm" \
		${_LLVM_BUILD_ARGS}
	ninja \
		-C "${CLANDRO_PKG_BUILDDIR}/build-llvm" \
		-j "${CLANDRO_PKG_MAKE_PROCESSES}" \
		install

	cmake \
		-G Ninja \
		-S "${CLANDRO_PKG_CACHEDIR}/binaryen-${_BINARYEN_COMMIT}" \
		-B "${CLANDRO_PKG_BUILDDIR}/build-binaryen" \
		${_BINARYEN_BUILD_ARGS}
	ninja \
		-C "${CLANDRO_PKG_BUILDDIR}/build-binaryen" \
		-j "${CLANDRO_PKG_MAKE_PROCESSES}" \
		install
	...
	sed -e "s|-git||" -i "${CLANDRO_PKG_SRCDIR}/emscripten-version.txt"

	# skip using Makefile which does host npm install
	rm -fr "${CLANDRO_PREFIX}/opt/emscripten"
	./tools/install.py "${CLANDRO_PREFIX}/opt/emscripten"

	# remove node_modules directory
	# to be installed on device post install
	rm -fr "${CLANDRO_PREFIX}/opt/emscripten/node_modules"

	# subpackage optional third party test suite files
	cp -fr "${CLANDRO_PKG_SRCDIR}/test/third_party" "${CLANDRO_PREFIX}/opt/emscripten/test/third_party"

	# first run generates .emscripten and exits immediately
	rm -f "${CLANDRO_PKG_SRCDIR}/.emscripten"
	./emcc --generate-config
	sed \
		-e "s|^EMSCRIPTEN_ROOT.*|EMSCRIPTEN_ROOT = '${CLANDRO_PREFIX}/opt/emscripten' # directory|" \
		-e "s|^LLVM_ROOT.*|LLVM_ROOT = '${CLANDRO_PREFIX}/opt/emscripten-llvm/bin' # directory|" \
		-e "s|^BINARYEN_ROOT.*|BINARYEN_ROOT = '${CLANDRO_PREFIX}/opt/emscripten-binaryen' # directory|" \
		-e "s|^NODE_JS.*|NODE_JS = '${CLANDRO_PREFIX}/bin/node' # executable|" \
		-i .emscripten
	grep "${CLANDRO_PREFIX}" "${CLANDRO_PKG_SRCDIR}/.emscripten"
	install -Dm644 "${CLANDRO_PKG_SRCDIR}/.emscripten" "${CLANDRO_PREFIX}/opt/emscripten/.emscripten"

	# add emscripten directory to PATH var
	cat <<- EOF > "${CLANDRO_PKG_TMPDIR}/emscripten.sh"
	#!${CLANDRO_PREFIX}/bin/sh
	export PATH=\${PATH}:${CLANDRO_PREFIX}/opt/emscripten
	EOF
	install -Dm644 "${CLANDRO_PKG_TMPDIR}/emscripten.sh" "${CLANDRO_PREFIX}/etc/profile.d/emscripten.sh"
	...
	for tool in llvm-{addr2line,dwarfdump,dwp,link,nm,objdump,ranlib,readobj,size,strings}; do
		install -Dm755 "${CLANDRO_PKG_BUILDDIR}/build-llvm/bin/${tool}" "${CLANDRO_PREFIX}/opt/emscripten-llvm/bin/${tool}"
	done
...
clandro_step_post_massage() {
	local upstream_bin=$(ls "${CLANDRO_PKG_CACHEDIR}/emsdk/upstream/bin")
	local llvm_bin=$(ls "${CLANDRO_TOPDIR}/emscripten/subpackages/emscripten-llvm/massage/${CLANDRO_PREFIX_CLASSICAL}/opt/emscripten-llvm/bin")
	local binaryen_bin=$(ls "${CLANDRO_TOPDIR}/emscripten/subpackages/emscripten-binaryen/massage/${CLANDRO_PREFIX_CLASSICAL}/opt/emscripten-binaryen/bin")
	local df=$(diff -u <(echo "${upstream_bin}") <(echo -e "${llvm_bin}\n${binaryen_bin}" | sort))
	if [[ -n "${df}" ]]; then
		clandro_error_exit "Mismatch list of binaries with upstream:\n${df}"
	fi

	local upstream_entrypoint=$(find "${CLANDRO_PKG_CACHEDIR}/emsdk/upstream/emscripten" -mindepth 1 -maxdepth 1 -type f | xargs -i bash -c "[[ -x '{}' ]] && basename '{}'" | sort)
	local downstream_entrypoint=$(find "${CLANDRO_PREFIX}/opt/emscripten" -mindepth 1 -maxdepth 1 -type f | xargs -i bash -c "[[ -x '{}' ]] && basename '{}'" | sort)
	local df2=$(diff -u <(echo "${upstream_entrypoint}") <(echo "${downstream_entrypoint}"))
	if [[ -n "${df2}" ]]; then
		clandro_error_exit "Mismatch list of entrypoints with upstream:\n${df2}"
	fi
}
...
clandro_step_create_debscripts() {
	...
	sed \
		...
		"${CLANDRO_PKG_BUILDER_DIR}/postinst.sh" > postinst
	sed \
		-e "s|@CLANDRO_PREFIX@|${CLANDRO_PREFIX}|g" \
		"${CLANDRO_PKG_BUILDER_DIR}/postrm.sh" > postrm
