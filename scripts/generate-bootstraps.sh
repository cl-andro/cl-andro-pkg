...
CLANDRO_ARCHITECTURES=("aarch64" "arm" "i686" "x86_64")

# The supported clandro package managers.
CLANDRO_PACKAGE_MANAGERS=("apt" "pacman")
...
REPO_BASE_URL="${REPO_BASE_URLS[${CLANDRO_PACKAGE_MANAGER}]}"
...
pull_package() {
	...
	if [ ${CLANDRO_PACKAGE_MANAGER} = "apt" ]; then
		local package_url
		...
		if [ ! -e "$package_tmpdir/package.deb" ]; then
			...
			(cd "$package_tmpdir"
				...
				if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE}; then
					...
					tar tf "$data_archive" | sed -E -e 's@^\./@/@' -e 's@^/$@/.@' -e 's@^([^./])@/\1@' > "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/info/${package_name}.list"

					# Generate checksums (md5).
					tar xf "$data_archive"
					find data -type f -print0 | xargs -0 -r md5sum | sed 's@^\.$@@g' > "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/info/${package_name}.md5sums"
					...
					for file in conffiles postinst postrm preinst prerm; do
						if [ -f "${PWD}/${file}" ]; then
							cp "$file" "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/dpkg/info/${package_name}.${file}"
						fi
					done
				fi
			)
		...
		if [ ! -e "$package_tmpdir/package.pkg.tar.xz" ]; then
			...
			(cd "$package_tmpdir"
				...
				mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/pacman/local/${package_desc}"
				{
					echo "%FILES%"
					tar xvf package.pkg.tar.xz -C "$BOOTSTRAP_ROOTFS" .INSTALL .MTREE data 2> /dev/null | grep '^data/' || true
				} >> "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/pacman/local/${package_desc}/files"
				mv "${BOOTSTRAP_ROOTFS}/.MTREE" "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/pacman/local/${package_desc}/mtree"
				if [ -f "${BOOTSTRAP_ROOTFS}/.INSTALL" ]; then
					mv "${BOOTSTRAP_ROOTFS}/.INSTALL" "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/pacman/local/${package_desc}/install"
				fi
				{
					...
					jq -r -j '."'${package_name}'" | to_entries | .[] | select(.key | contains('$(sed 's/^/"/; s/ /","/g; s/$/"/' <<< ${keys_desc})')) | "%",(if .key == "ISIZE" then "SIZE" else .key end),"%\n",.value,"\n\n" | if type == "array" then (.| join("\n")) else . end' \
						"${PATH_DB_PACKAGES}"
				} >> "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/pacman/local/${package_desc}/desc"
			)
		fi
	fi
}
...
add_clandro_bootstrap_second_stage_files() {
	...
	echo "[*] Adding clandro bootstrap second stage files..."
	...
	# TODO: Remove it when Clandro app supports `pacman` bootstraps installation.
	sed -e "s|@CLANDRO_PREFIX@|${CLANDRO_PREFIX}|g" \
...
show_usage() {
	...
	echo
	echo "Generate bootstrap archives for Clandro application."
	echo
	echo "Options:"
	echo
	...
	echo
	echo "Architectures: ${CLANDRO_ARCHITECTURES[*]}"
	echo "Repository Base Url: ${REPO_BASE_URL}"
	echo "Prefix: ${CLANDRO_PREFIX}"
        echo "Package manager: ${CLANDRO_PACKAGE_MANAGER}"
	echo
}
...
if [[ "$CLANDRO_PACKAGE_MANAGER" == *" "* ]] || [[ " ${CLANDRO_PACKAGE_MANAGERS[*]} " != *" $CLANDRO_PACKAGE_MANAGER "* ]]; then
	echo "[!] Invalid package manager '$CLANDRO_PACKAGE_MANAGER'" 1>&2
	echo "Supported package managers: '${CLANDRO_PACKAGE_MANAGERS[*]}'" 1>&2
	exit 1
fi
...
for package_arch in "${CLANDRO_ARCHITECTURES[@]}"; do
	...
	if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE}; then
		if [ ${CLANDRO_PACKAGE_MANAGER} = "apt" ]; then
			...
			mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/pacman/sync"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/pacman/local"
			echo "9" >> "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/lib/pacman/local/ALPM_DB_VERSION"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/cache/pacman/pkg"
			mkdir -p "${BOOTSTRAP_ROOTFS}/${CLANDRO_PREFIX}/var/log"
		fi
	fi
	...
	# Package manager.
	if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE}; then
		pull_package ${CLANDRO_PACKAGE_MANAGER}
	fi

	# Core utilities.
	pull_package bash # Used by `clandro-bootstrap-second-stage.sh`
	pull_package bzip2
	# if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE}; then
	...
	pull_package sed
	pull_package tar
	pull_package clandro-core
	pull_package clandro-exec
	pull_package clandro-keyring
	pull_package clandro-tools
