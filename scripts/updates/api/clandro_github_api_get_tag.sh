...
clandro_github_api_get_tag() {
	if [[ -z "${GITHUB_TOKEN:-}" ]]; then
		...
		clandro_error_exit "GITHUB_TOKEN environment variable not set."
	fi
	...
	#     user="vim"
	#     repo="vim"
	#        _="archive/refs/tags/v${CLANDRO_PKG_VERSION}.tar.gz"
	IFS='/' read -r _ _ _ user repo _ <<< "${CLANDRO_PKG_SRCURL}"
	project="${user}/${repo}"
