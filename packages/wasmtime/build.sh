CLANDRO_PKG_HOMEPAGE=https://wasmtime.dev/
CLANDRO_PKG_DESCRIPTION="A standalone runtime for WebAssembly"
CLANDRO_PKG_LICENSE="Apache-2.0"
CLANDRO_PKG_LICENSE_FILE="LICENSE"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION="44.0.1"
CLANDRO_PKG_SRCURL=git+https://github.com/bytecodealliance/wasmtime
...
clandro_pkg_auto_update() {
	...
	if [[ "${latest_version}" == "v${CLANDRO_PKG_VERSION}" ]]; then
	...
	clandro_pkg_upgrade_version "${latest_version/v/}"
}
...
clandro_step_pre_configure() {
	...
	local patch="$CLANDRO_PKG_BUILDER_DIR/listenfd-32-bit-android.diff"
...
clandro_step_make_install() {
	install -Dm755 -t "${CLANDRO_PREFIX}/bin" "target/${CARGO_TARGET_NAME}/release/wasmtime"
}
