...
ci_artifact_url() {
	curl --silent \
		-H "Authorization: token ${GITHUB_TOKEN}" \
		-H "Accept: application/vnd.github.v3+json" \
		"https://api.github.com/repos/clandro/clandro-packages/actions/runs/${1}/artifacts" \
...
download_ci_artifacts() {
	...
	CI_ARTIFACT_ZIP="$HOME/.clandro-build/_cache/artifact-${WORKFLOW_ID}.zip"
...
[[ "${GITHUB_EVENT_NAME:-}" == "push" ]] && {
	...
	readarray -t CLANDRO_PACKAGE_DIRECTORIES < <(jq --raw-output 'del(.pkg_format) | keys | .[]' repo.json) || :

	# We should obtain data about all commits in this push to check that they are from the same PR if any
	RELATED_PRS_QUERY="
	query {
		repository(owner: \"termux\", name: \"clandro-packages\") {
		$(
	...
	DIRS_REGEX="$(paste -sd'|' <<< "${CLANDRO_PACKAGE_DIRECTORIES[@]}")" || exit 0
	...
	readarray -t PR_CHANGED_PACKAGES_DEPS < <(
		for dep in "${PR_CHANGED_PACKAGES[@]:-}"; do
			./scripts/buildorder.py "$dep" "${CLANDRO_PACKAGE_DIRECTORIES[@]}" 2>/dev/null | awk '{print $NF}'
		done | sort -u
	) || :
