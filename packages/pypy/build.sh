...
CLANDRO_PKG_DESCRIPTION="A fast, compliant alternative implementation of Python"
CLANDRO_PKG_LICENSE="MIT"
CLANDRO_PKG_MAINTAINER="@licy183"
_MAJOR_VERSION=2.7
CLANDRO_PKG_VERSION="7.3.22"
CLANDRO_PKG_SRCURL=(
	https://downloads.python.org/pypy/pypy$_MAJOR_VERSION-v$CLANDRO_PKG_VERSION-src.tar.bz2
	https://downloads.python.org/pypy/pypy2.7-v$CLANDRO_PKG_VERSION-linux64.tar.bz2
	https://downloads.python.org/pypy/pypy2.7-v$CLANDRO_PKG_VERSION-linux32.tar.bz2
)
CLANDRO_PKG_SHA256=(
...
CLANDRO_PKG_BUILD_DEPENDS="aosp-libs, tk, xorgproto"
CLANDRO_PKG_RECOMMENDS="clang, make, pkg-config"
CLANDRO_PKG_SUGGESTS="pypy-tkinter"
CLANDRO_PKG_BUILD_IN_SRC=true

clandro_step_post_get_source() {
	local p="$CLANDRO_PKG_BUILDER_DIR/9999-add-ANDROID_API_LEVEL-for-sysconfigdata.diff"
...
__setup_host_pypy2() {
	export PATH="$CLANDRO_PKG_SRCDIR/pypy2.7-v$CLANDRO_PKG_VERSION-linux$CLANDRO_ARCH_BITS/bin:$PATH"
...
"
	...
	case "$CLANDRO_ARCH" in
		"aarch64" |  "arm")
			PROOT_TARGET+=" -q $CLANDRO_PKG_CACHEDIR/qemu-static-bin/qemu-$CLANDRO_ARCH-static"
	...
	cp ./pypy-c "$CLANDRO_PKG_SRCDIR"/pypy/goal/pypy-c
	cp ./libpypy-c.so "$CLANDRO_PKG_SRCDIR"/pypy/goal/libpypy-c.so

	# Dummy cc and strip
	mkdir -p "$CLANDRO_PKG_SRCDIR"/dummy-bin
	cp "$CLANDRO_PKG_BUILDER_DIR"/cc.sh "$CLANDRO_PKG_SRCDIR"/dummy-bin/cc
	chmod +x "$CLANDRO_PKG_SRCDIR"/dummy-bin/cc
	ln -sf $(command -v llvm-strip) "$CLANDRO_PKG_SRCDIR"/dummy-bin/strip

	# Build cffi imports (Cross exec)
	$PROOT_TARGET env -i \
				PATH="$CLANDRO_PKG_SRCDIR/dummy-bin:$PATH" \
				...
				LDFLAGS="$LDFLAGS" \
				"$CLANDRO_PKG_SRCDIR"/pypy/goal/pypy-c \
...
clandro_step_make_install() {
	rm -rf $CLANDRO_PREFIX/opt/pypy
	unzip -d $CLANDRO_PREFIX/opt/ pypy$_MAJOR_VERSION-v$CLANDRO_PKG_VERSION.zip
	mv $CLANDRO_PREFIX/opt/pypy$_MAJOR_VERSION-v$CLANDRO_PKG_VERSION $CLANDRO_PREFIX/opt/pypy
	ln -sfr $CLANDRO_PREFIX/opt/pypy/bin/pypy $CLANDRO_PREFIX/bin/
	ln -sfr $CLANDRO_PREFIX/opt/pypy/bin/libpypy-c.so $CLANDRO_PREFIX/lib/
}
...
clandro_step_create_debscripts() {
	...
	rm -Rf $CLANDRO_PREFIX/opt/pypy/site-packages/*

	echo "Deleting *.pyc..."
	find $CLANDRO_PREFIX/opt/pypy/lib-python/ | grep -E "(__pycache__|\.pyc|\.pyo$)" | xargs rm -rf
	find $CLANDRO_PREFIX/opt/pypy/lib_pypy/ | grep -E "(__pycache__|\.pyc|\.pyo$)" | xargs rm -rf
