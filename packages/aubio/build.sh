CLANDRO_PKG_HOMEPAGE=https://aubio.org/
CLANDRO_PKG_DESCRIPTION="A library to label music and sounds"
CLANDRO_PKG_LICENSE="GPL-3.0"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION=0.4.9
CLANDRO_PKG_REVISION=7
CLANDRO_PKG_SRCURL=https://aubio.org/pub/aubio-${CLANDRO_PKG_VERSION}.tar.bz2
CLANDRO_PKG_SHA256=d48282ae4dab83b3dc94c16cf011bcb63835c1c02b515490e1883049c3d1f3da
CLANDRO_PKG_DEPENDS="ffmpeg, libsamplerate, libsndfile"
CLANDRO_PKG_BUILD_IN_SRC=true

clandro_step_pre_configure() {
	declare -a _commits=(
	8a05420e
	cdfe9cef
	245deead
	0b947f96
	53bc55cd
	)

	declare -a _checksums=(
	
	)

	for i in "${!_commits[@]}"; do
		PATCHFILE="${CLANDRO_PKG_CACHEDIR}/aubio_patch_${_commits[i]}.patch"
		clandro_download \
			"https://github.com/aubio/aubio/commit/${_commits[i]}.patch" \
			"$PATCHFILE" \
			"${_checksums[i]}"
		patch -p1 -i "$PATCHFILE"
	done

	patch -p1 -i "${CLANDRO_PKG_BUILDER_DIR}"/src-io-source_avcodec.c.diff

	CPPFLAGS+=" -DFF_API_LAVF_AVCTX"
	sed -i 's#AV_INPUT_BUFFER_MIN_SIZE#16384#' src/io/source_avcodec.c
}

clandro_step_configure() {
	./waf configure \
		--prefix=$CLANDRO_PREFIX \
		LINKFLAGS="$LDFLAGS" \
		$CLANDRO_PKG_EXTRA_CONFIGURE_ARGS
}

clandro_step_make() {
	./waf
}

clandro_step_make_install() {
	./waf install
}
