clandro_setup_toolchain_23c() {
	...
	if [ -d $CLANDRO_STANDALONE_TOOLCHAIN ]; then
		return
	fi
	...
	local _CLANDRO_TOOLCHAIN_TMPDIR=${CLANDRO_STANDALONE_TOOLCHAIN}-tmp
	rm -Rf "$_CLANDRO_TOOLCHAIN_TMPDIR"
	...
	cp $NDK/toolchains/llvm/prebuilt/linux-x86_64 $_CLANDRO_TOOLCHAIN_TMPDIR -r

	# Remove android-support header wrapping not needed on android-21:
	rm -Rf "$_CLANDRO_TOOLCHAIN_TMPDIR/sysroot/usr/local"

	for HOST_PLAT in aarch64-linux-android armv7a-linux-androideabi i686-linux-android x86_64-linux-android; do
		cp $_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT$CLANDRO_PKG_API_LEVEL-clang \
			$_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT-clang
		cp $_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT$CLANDRO_PKG_API_LEVEL-clang++ \
			$_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT-clang++

		cp $_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT$CLANDRO_PKG_API_LEVEL-clang \
			$_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT-cpp
		sed -i 's/clang/clang -E/' \
			$_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT-cpp

		cp $_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT-clang \
			$_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT-gcc
		cp $_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT-clang++ \
			$_CLANDRO_TOOLCHAIN_TMPDIR/bin/$HOST_PLAT-g++
	done

	cp $_CLANDRO_TOOLCHAIN_TMPDIR/bin/armv7a-linux-androideabi$CLANDRO_PKG_API_LEVEL-clang \
		$_CLANDRO_TOOLCHAIN_TMPDIR/bin/arm-linux-androideabi-clang
	cp $_CLANDRO_TOOLCHAIN_TMPDIR/bin/armv7a-linux-androideabi$CLANDRO_PKG_API_LEVEL-clang++ \
		$_CLANDRO_TOOLCHAIN_TMPDIR/bin/arm-linux-androideabi-clang++
	cp $_CLANDRO_TOOLCHAIN_TMPDIR/bin/armv7a-linux-androideabi-cpp \
		$_CLANDRO_TOOLCHAIN_TMPDIR/bin/arm-linux-androideabi-cpp

	# rust 1.75.0+ expects this directory to be present
	rm -fr "${_CLANDRO_TOOLCHAIN_TMPDIR}"/toolchains
	mkdir -p "${_CLANDRO_TOOLCHAIN_TMPDIR}"/toolchains/llvm/prebuilt
	ln -fs ../../.. "${_CLANDRO_TOOLCHAIN_TMPDIR}"/toolchains/llvm/prebuilt/linux-x86_64
	...
	cat > $_CLANDRO_TOOLCHAIN_TMPDIR/bin/pkg-config <<-HERE
		#!/bin/sh
		export PKG_CONFIG_DIR=
		export PKG_CONFIG_LIBDIR=$PKG_CONFIG_LIBDIR
		exec $_HOST_PKGCONFIG "\$@"
	HERE
	chmod +x "$_CLANDRO_TOOLCHAIN_TMPDIR"/bin/pkg-config

	cd $_CLANDRO_TOOLCHAIN_TMPDIR/sysroot
	...
	grep -lrw $_CLANDRO_TOOLCHAIN_TMPDIR/sysroot/usr/include/c++/v1 -e 'include <version>' | xargs -n 1 sed -i 's/include <version>/include \"version\"/g'
	mv $_CLANDRO_TOOLCHAIN_TMPDIR $CLANDRO_STANDALONE_TOOLCHAIN
}
