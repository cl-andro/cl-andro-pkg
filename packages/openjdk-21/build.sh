...
CLANDRO_PKG_VERSION="21.0.11"
CLANDRO_PKG_SRCURL="https://github.com/openjdk/jdk21u/archive/refs/tags/jdk-${CLANDRO_PKG_VERSION}-ga.tar.gz"
CLANDRO_PKG_SHA256=76b8310966649ea8a6340f92d4f19f6f84e3083b682a514c8f1999c93373385f
CLANDRO_PKG_AUTO_UPDATE=true
CLANDRO_PKG_UPDATE_VERSION_REGEXP='21\.\d+\.\d+(?=-ga)'
...
CLANDRO_PKG_RECOMMENDS="ca-certificates-java, openjdk-21-x, resolv-conf"
...
clandro_step_configure() {
	local jdk_ldflags="-L${CLANDRO_PREFIX}/lib \
		-Wl,-rpath=$CLANDRO_PREFIX/lib/jvm/java-21-openjdk/lib \
...
clandro_step_make_install() {
	rm -rf  $CLANDRO_PREFIX/lib/jvm/java-21-openjdk
	mkdir -p $CLANDRO_PREFIX/lib/jvm/java-21-openjdk
	cp -r build/linux-${CLANDRO_ARCH/i686/x86}-server-release/images/jdk/* \
		$CLANDRO_PREFIX/lib/jvm/java-21-openjdk/
	find $CLANDRO_PREFIX/lib/jvm/java-21-openjdk/ -name "*.debuginfo" -delete

	# Dependent projects may need JAVA_HOME.
	mkdir -p $CLANDRO_PREFIX/lib/jvm/java-21-openjdk/etc/profile.d
	echo "export JAVA_HOME=$CLANDRO_PREFIX/lib/jvm/java-21-openjdk/" > \
		$CLANDRO_PREFIX/lib/jvm/java-21-openjdk/etc/profile.d/java.sh
}

clandro_step_post_make_install() {
	cd $CLANDRO_PREFIX/lib/jvm/java-21-openjdk/man/man1
	for manpage in *.1; do
		gzip "$manpage"
	done

	# Make sure that our alternatives file is up to date.
	binaries="$(find $CLANDRO_PREFIX/lib/jvm/java-21-openjdk/bin -executable -type f | xargs -I{} basename "{}" | xargs echo)"
	manpages="$(find $CLANDRO_PREFIX/lib/jvm/java-21-openjdk/man/man1 -name "*.1.gz" | xargs -I{} basename "{}" | xargs echo)"

	local failure=false
	for binary in $binaries; do
		grep -q "lib/jvm/java-21-openjdk/bin/${binary}$" "$CLANDRO_PKG_BUILDER_DIR"/openjdk-21.alternatives || {
			echo "ERROR: Missing entry for binary: $binary in openjdk-21.alternatives"
			failure=true
		}
	done
	for manpage in $manpages; do
		grep -q "lib/jvm/java-21-openjdk/man/man1/${manpage}$" "$CLANDRO_PKG_BUILDER_DIR"/openjdk-21.alternatives || {
			echo "ERROR: Missing entry for manpage: $manpage in openjdk-21.alternatives"
			failure=true
		}
	done
	if [[ "$failure" = true ]]; then
		clandro_error_exit "openjdk-21.alternatives is not up to date, please update it."
	fi
}
