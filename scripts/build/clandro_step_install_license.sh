...
clandro_step_install_license() {
	...
	if [[ -n "${CLANDRO_PKG_LICENSE_FILE}" ]]; then
		COUNTER=1
		...
		while read -r LICENSE; do
			...
			[[ -f "$CLANDRO_PKG_SRCDIR/$LICENSE" ]] || {
				clandro_error_exit "$CLANDRO_PKG_SRCDIR/$LICENSE does not exist"
			}
			...
			if [[ -n ${INSTALLED_LICENSES[${LICENSE_FILEPATH}]:-} ]]; then
				...
				TARGET="$CLANDRO_PREFIX/share/doc/${CLANDRO_PKG_NAME}/${LICENSE_FILEPATH}.$((COUNTER++))"
			else
				TARGET="$CLANDRO_PREFIX/share/doc/${CLANDRO_PKG_NAME}/${LICENSE_FILEPATH}"
			...
			cp -f "${CLANDRO_PKG_SRCDIR}/${LICENSE}" "$TARGET"
		done <<< "${CLANDRO_PKG_LICENSE_FILE//,/$'\n'}"
				...
				|'PythonPL'|'X11'|'ZLIB')
					...
					if (( ! FROM_SOURCES )); then
						...
						for FILE in "${COMMON_LICENSE_FILES[@]}" "${EXTRA_LICENSE_FILES[@]}"; do
							[[ -f "$CLANDRO_PKG_SRCDIR/$FILE" ]] && {
								if (( COUNTER )); then
									cp -f "${CLANDRO_PKG_SRCDIR}/$FILE" "${CLANDRO_PREFIX}/share/doc/${CLANDRO_PKG_NAME}/copyright.${COUNTER}"
								else
									cp -f "${CLANDRO_PKG_SRCDIR}/$FILE" "${CLANDRO_PREFIX}/share/doc/${CLANDRO_PKG_NAME}/copyright"
								fi
						...
						if (( ! FROM_SOURCES )); then
							clandro_error_exit "${CLANDRO_PKG_NAME}: Could not find a license file for $LICENSE in the package sources"
						fi
					fi
				;;
				*) # For the rest we can use a link to the generic license file
					[[ -f "$CLANDRO_SCRIPTDIR/packages/clandro-licenses/LICENSES/${LICENSE}.txt" ]] || {
						# If we get here, no license file could be found
						clandro_error_exit "${CLANDRO_PKG_NAME}: Could not find a license file for $LICENSE in packages/clandro-licenses"
					}
					...
					case "$CLANDRO_PACKAGE_LIBRARY" in
						...
						*)        clandro_error_exit "'$CLANDRO_PACKAGE_LIBRARY' is not a supported libc";;
					esac
					if (( COUNTER )); then
						ln -sf "$TO_LICENSE" "$CLANDRO_PREFIX/share/doc/$CLANDRO_PKG_NAME/copyright.${COUNTER}"
					else
						ln -sf "$TO_LICENSE" "$CLANDRO_PREFIX/share/doc/$CLANDRO_PKG_NAME/copyright"
					fi
					(( ++COUNTER ))
				;;
			esac
		done <<< "${CLANDRO_PKG_LICENSE//,/$'\n'}"
		local license_files
		license_files="$(find -L "$CLANDRO_PREFIX/share/doc/$CLANDRO_PKG_NAME" -maxdepth 1 \( -type f -o -type l \) -name "copyright*")"
		[[ -n "$license_files" ]] || {
			clandro_error_exit "No LICENSE file was installed for $CLANDRO_PKG_NAME"
		}
	fi
return 0
}
