CLANDRO_PKG_HOMEPAGE=https://clang.llvm.org/
CLANDRO_PKG_DESCRIPTION="Modular compiler and toolchain technologies library"
CLANDRO_PKG_LICENSE="Apache-2.0, NCSA"
CLANDRO_PKG_LICENSE_FILE="llvm/LICENSE.TXT"
CLANDRO_PKG_MAINTAINER="@finagolfin"
# Keep flang version and revision in sync when updating (enforced by check in clandro_step_pre_configure).
CLANDRO_PKG_VERSION="21.1.8"
CLANDRO_PKG_REVISION=2
CLANDRO_PKG_SHA256=4633a23617fa31a3ea51242586ea7fb1da7140e426bd62fc164261fe036aa142
CLANDRO_PKG_AUTO_UPDATE=false
CLANDRO_PKG_SRCURL="https://github.com/llvm/llvm-project/releases/download/llvmorg-${CLANDRO_PKG_VERSION}/llvm-project-${CLANDRO_PKG_VERSION}.src.tar.xz"
CLANDRO_PKG_HOSTBUILD=true
CLANDRO_PKG_RM_AFTER_INSTALL="
bin/ld64.lld.darwin*
lib/libgomp.a
lib/libiomp5.a
share/man/man1/lit.1
"
CLANDRO_PKG_DEPENDS="libc++, libffi, libxml2, ncurses, zlib, zstd"
# Replace gcc since gcc is deprecated by google on android and is not maintained upstream.
# Conflict with clang versions earlier than 3.9.1-3 since they bundled llvm.
CLANDRO_PKG_CONFLICTS="gcc, clang (<< 3.9.1-3)"
CLANDRO_PKG_BREAKS="libclang, libclang-dev, libllvm-dev"
CLANDRO_PKG_REPLACES="gcc, libclang, libclang-dev, libllvm-dev"
CLANDRO_PKG_GROUPS="base-devel"

# See https://llvm.org/docs/CMake.html:
CLANDRO_PKG_EXTRA_CONFIGURE_ARGS="
-DANDROID_PLATFORM_LEVEL=$CLANDRO_PKG_API_LEVEL
-DLLVM_ENABLE_PIC=ON
-DLLVM_ENABLE_LIBEDIT=OFF
-DLLVM_INCLUDE_TESTS=OFF
-DCLANG_DEFAULT_CXX_STDLIB=libc++
-DCLANG_DEFAULT_LINKER=lld
-DCLANG_INCLUDE_TESTS=OFF
-DCLANG_TOOL_C_INDEX_TEST_BUILD=OFF
-DCOMPILER_RT_USE_BUILTINS_LIBRARY=ON
-DLLVM_LINK_LLVM_DYLIB=ON
-DLLDB_ENABLE_PYTHON=ON
-DLLDB_PYTHON_RELATIVE_PATH=lib/python${CLANDRO_PYTHON_VERSION}/site-packages
-DLLDB_PYTHON_EXE_RELATIVE_PATH=bin/python${CLANDRO_PYTHON_VERSION}
-DLLDB_PYTHON_EXT_SUFFIX=.cpython-${CLANDRO_PYTHON_VERSION//./}.so
-DLLVM_NATIVE_TOOL_DIR=$CLANDRO_PKG_HOSTBUILD_DIR/bin
-DCROSS_TOOLCHAIN_FLAGS_LLVM_NATIVE=-DLLVM_NATIVE_TOOL_DIR=$CLANDRO_PKG_HOSTBUILD_DIR/bin
-DLIBOMP_ENABLE_SHARED=FALSE
-DOPENMP_ENABLE_LIBOMPTARGET=OFF
-DLLVM_ENABLE_SPHINX=ON
-DSPHINX_OUTPUT_MAN=ON
-DSPHINX_WARNINGS_AS_ERRORS=OFF
-DLLVM_TARGETS_TO_BUILD=all
-DLLVM_EXPERIMENTAL_TARGETS_TO_BUILD=ARC;CSKY;M68k;VE
-DPERL_EXECUTABLE=$(command -v perl)
-DLLVM_ENABLE_FFI=ON
-DLLVM_ENABLE_RTTI=ON
-DLLVM_INSTALL_UTILS=ON
-DLLVM_INSTALL_BINUTILS_SYMLINKS=ON
-DMLIR_INSTALL_AGGREGATE_OBJECTS=OFF
"

if (( CLANDRO_ARCH_BITS == 32 )); then
	# Do not set _FILE_OFFSET_BITS=64
	CLANDRO_PKG_EXTRA_CONFIGURE_ARGS+=" -DLLVM_FORCE_SMALLFILE_FOR_ANDROID=on"
fi

CLANDRO_PKG_FORCE_CMAKE=true
CLANDRO_PKG_HAS_DEBUG=false
# Debug build succeeds but make install with:
# cp: cannot stat '../src/projects/openmp/runtime/exports/common.min.50.ompt.optional/include/omp.h': No such file or directory
# common.min.50.ompt.optional should be common.deb.50.ompt.optional when doing debug build

# shellcheck disable=SC2030,2031
clandro_step_post_get_source() {
	# Version guard to keep flang in sync
	local flang_version flang_revision
	flang_version="$(. "$CLANDRO_SCRIPTDIR/packages/flang/build.sh"; echo "${CLANDRO_PKG_VERSION}")"
	flang_revision="$(CLANDRO_PKG_REVISION=0; . "$CLANDRO_SCRIPTDIR/packages/flang/build.sh"; echo "${CLANDRO_PKG_REVISION}")"
	if [[ "${flang_version}-${flang_revision}" != "${CLANDRO_PKG_VERSION}-${CLANDRO_PKG_REVISION:-0}" ]]; then
		clandro_error_exit "Version mismatch between libllvm and flang. libllvm=$CLANDRO_PKG_VERSION-$CLANDRO_PKG_REVISION, flang=$flang_version-$flang_revision"
	fi
}

# shellcheck disable=SC2031
clandro_step_host_build() {
	clandro_setup_cmake
	clandro_setup_ninja

	cmake -G Ninja -DCMAKE_BUILD_TYPE=Release \
		-DLLVM_ENABLE_PROJECTS='clang;clang-tools-extra;lldb;mlir' "$CLANDRO_PKG_SRCDIR/llvm"
	ninja -j "$CLANDRO_PKG_MAKE_PROCESSES" clang-tblgen clang-tidy-confusable-chars-gen \
		lldb-tblgen llvm-tblgen mlir-tblgen mlir-linalg-ods-yaml-gen
}

# shellcheck disable=SC2031
clandro_step_pre_configure() {
	# Add unknown vendor, otherwise it screws with the default LLVM triple
	# detection.
	local llvm_default_target_triple="${CCCLANDRO_HOST_PLATFORM/-/-unknown-}"
	local llvm_target_arch
	case "$CLANDRO_ARCH" in
		"aarch64") llvm_target_arch="AArch64";;
		"arm") llvm_target_arch="ARM";;
		"i686"|"x86_64") llvm_target_arch="X86";;
		*) clandro_error_exit "Invalid arch: $CLANDRO_ARCH";;
	esac

	local default_sysroot
	if [[ "$CLANDRO__PREFIX" == "$CLANDRO__ROOTFS" ]]; then
		default_sysroot=".."
	else
		default_sysroot="../.."
	fi
	CLANDRO_PKG_EXTRA_CONFIGURE_ARGS+=" -DDEFAULT_SYSROOT=$default_sysroot"

	local llvm_projects="clang;clang-tools-extra;compiler-rt;lld;lldb;mlir;openmp;polly"
	CLANDRO_PKG_EXTRA_CONFIGURE_ARGS+=" -DLLVM_ENABLE_PROJECTS=$llvm_projects"

	# see CMakeLists.txt and tools/clang/CMakeLists.txt
	CLANDRO_PKG_EXTRA_CONFIGURE_ARGS+=" -DLLVM_TARGET_ARCH=$llvm_target_arch"
	CLANDRO_PKG_EXTRA_CONFIGURE_ARGS+=" -DLLVM_HOST_TRIPLE=$llvm_default_target_triple"
	export CLANDRO_SRCDIR_SAVE="$CLANDRO_PKG_SRCDIR"
	CLANDRO_PKG_SRCDIR="$CLANDRO_PKG_SRCDIR/llvm"
}

clandro_step_post_configure() {
	CLANDRO_PKG_SRCDIR="$CLANDRO_SRCDIR_SAVE"
	unset CLANDRO_SRCDIR_SAVE
}

# shellcheck disable=SC2031
clandro_step_post_make_install() {
	if [[ "$CLANDRO_PKG_CMAKE_BUILD" == "Ninja" ]]; then
		ninja -j "$CLANDRO_PKG_MAKE_PROCESSES" docs-{llvm,clang}-man
	else
		make -j "$CLANDRO_PKG_MAKE_PROCESSES" docs-{llvm,clang}-man
	fi

	cp docs/man/* "$CLANDRO_PREFIX/share/man/man1"
	cp tools/clang/docs/man/{clang,diagtool}.1 "$CLANDRO_PREFIX/share/man/man1"
	cd "$CLANDRO_PREFIX/bin" || clandro_error_exit "failed to change into 'bin' directory"


	for tool in clang clang++ cc c++ cpp gcc g++; do
		ln -f -s "clang-${CLANDRO_PKG_VERSION%%.*}" "$tool"
	done

	ln -f -s clang++ "clang++-${CLANDRO_PKG_VERSION%%.*}"
	ln -f -s "${CLANDRO_PKG_VERSION%%.*}" "$CLANDRO_PREFIX/lib/clang/latest"

	# Instead of symlinks, for executables named after target triplets, create the same type of
	# wrapper that the cross-compiling NDK uses to choose the correct target, including the API level
	local target="$CCCLANDRO_HOST_PLATFORM"
	if [[ "$CLANDRO_ARCH" == "arm" ]]; then
		target="armv7a-linux-androideabi$CLANDRO_PKG_API_LEVEL"
	fi

	for tool in clang clang++ cpp gcc g++; do
		local wrapper="${CLANDRO_HOST_PLATFORM}-${tool}"
		rm -f "$wrapper"
		cat <<- EOF > "$wrapper"
		#!$CLANDRO_PREFIX/bin/bash
		if [ "\$1" != "-cc1" ]; then
			"\$(dirname \$0)/$tool" --target="$target" "\$@"
		else
			# Target is already an argument.
			"\$(dirname \$0)/$tool" "\$@"
		fi
		EOF
		chmod u+x "$wrapper"
	done

	# remove symbolic links to llvm-debuginfod and llvm-debuginfod-find
	# they conflict with elfutils, which is not necessary for most tasks
	rm -f "${CLANDRO_PREFIX}"/bin/debuginfod{,-find}

	# Make lld provide ld
	ln -sf ld.lld "${CLANDRO_PREFIX}/bin/ld"
	# https://bugs.llvm.org/show_bug.cgi?id=42455
	install -Dm644 -t "${CLANDRO_PREFIX}/share/man/man1" "$CLANDRO_PKG_SRCDIR/lld/docs/ld.lld.1"
	ln -sf ld.lld.1.gz "${CLANDRO_PREFIX}/share/man/man1/ld.1.gz"

	# Make llvm provide manpages for binutils that -DLLVM_INSTALL_BINUTILS_SYMLINKS=ON installed
	local -a _BINUTILS_CONFLICTING_WITH_LLVM=(
		"ar"
		"addr2line"
		"c++filt"
		"nm"
		"objcopy"
		"objdump"
		"ranlib"
		"readelf"
		"size"
		"strings"
		"strip"
	)
	local binutil
	for binutil in "${_BINUTILS_CONFLICTING_WITH_LLVM[@]}"; do
		ln -sf "llvm-${binutil//+/x}.1.gz" "${CLANDRO_PREFIX}/share/man/man1/${binutil}.1.gz"
	done

	# Make llvm and lld provide triplet-prefixed symbolic links which are used by some tools,
	# like python, perl and libtool:
	local -a _BINUTILS_WITH_HOST_PREFIX=(
		"ar"
		"ld"
		"nm"
		"objdump"
		"ranlib"
		"readelf"
		"strip"
	)
	for binutil in "${_BINUTILS_WITH_HOST_PREFIX[@]}"; do
		ln -sf "${binutil}" "${CLANDRO_PREFIX}/bin/${CLANDRO_HOST_PLATFORM}-${binutil}"
	done
}

# shellcheck disable=SC2031
clandro_step_pre_massage() {
	[[ "$CLANDRO_PACKAGE_FORMAT" != "pacman" ]] && return
	sed -i "s|@LLVM_MAJOR_VERSION@|${CLANDRO_PKG_VERSION%%.*}|g" ./share/libalpm/scripts/update-libcompiler-rt
}
