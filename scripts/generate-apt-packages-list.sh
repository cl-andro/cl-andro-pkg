...
CLANDRO_PACKAGES_DIR="$(realpath "$(dirname "$(realpath "$0")")/..")"
OUTPUT_DIR="$1"

readarray -t repo_paths <<< "$(jq --raw-output 'del(.pkg_format) | keys | .[]' "$CLANDRO_PACKAGES_DIR/repo.json")"

for arch in "aarch64" "arm" "i686" "x86_64"; do
	# Note that this is loop for generating the list of packages is being parallelized for each architecture
	for repo_path in "${repo_paths[@]}"; do
		repo_name="$(jq --raw-output ".\"$repo_path\".name" "$CLANDRO_PACKAGES_DIR/repo.json")"
		for pkg_path in "$CLANDRO_PACKAGES_DIR/$repo_path"/*; do
			(
				APT_VERSION=
				export CLANDRO_PKG_REVISION=0
				export CLANDRO_PKG_NO_STATICSPLIT=false
				set +euo pipefail
				...
				# Source: scripts/build/clandro_extract_dep_info.sh
				#	if [[ "$CLANDRO_PKG_REVISION" != "0" || "$CLANDRO_PKG_VERSION" != "${CLANDRO_PKG_VERSION/-/}" ]]; then
				#		VER_DEBIAN+="-$CLANDRO_PKG_REVISION"
				# fi
				if [[ "$CLANDRO_PKG_REVISION" != 0 || "$CLANDRO_PKG_VERSION" != "${CLANDRO_PKG_VERSION/-/}" ]]; then
					APT_VERSION="$CLANDRO_PKG_VERSION-$CLANDRO_PKG_REVISION"
				else
					APT_VERSION="$CLANDRO_PKG_VERSION"
				fi

				IFS="," read -r -a EXCLUDED_ARCHES <<< "${CLANDRO_PKG_EXCLUDED_ARCHES:-}"
				excluded=false
				...
				if [[ "$excluded" != true ]]; then
					...
					if [[ "$CLANDRO_PKG_NO_STATICSPLIT" == true ]]; then
						echo " false"
					else
						echo " true"
					fi
				...
				for subpkg in "$pkg_path"/*.subpackage.sh; do
					if [[ -f "$subpkg" ]]; then
						(
							set +euo pipefail
							export CLANDRO_SUBPKG_PLATFORM_INDEPENDENT=false
							. "$subpkg" &> /dev/null || :
							set -euo pipefail
							IFS="," read -r -a SUBPKG_EXCLUDED_ARCHES <<< "${CLANDRO_SUBPKG_EXCLUDED_ARCHES:-}"
							subpkg_excluded=false
							if [[ "${CLANDRO_SUBPKG_PLATFORM_INDEPENDENT}" == "false" ]]; then
