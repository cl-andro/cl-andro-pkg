CLANDRO_PKG_HOMEPAGE="https://jellyfin.org"
CLANDRO_PKG_DESCRIPTION="A free media system for organizing and streaming media (server)"
...
CLANDRO_PKG_SRCURL=(
	"https://github.com/jellyfin/jellyfin/archive/refs/tags/v${CLANDRO_PKG_VERSION[0]}.tar.gz"
	"https://github.com/jellyfin/jellyfin-web/archive/refs/tags/v${CLANDRO_PKG_VERSION[0]}.zip"
	"https://github.com/jellyfin/jellyfin-ffmpeg/archive/refs/tags/v${CLANDRO_PKG_VERSION[1]%.*}-${CLANDRO_PKG_VERSION[1]##*.}.tar.gz"
)
CLANDRO_PKG_SHA256=(
...
CLANDRO_PKG_BUILD_DEPENDS="aspnetcore-targeting-pack-9.0, dotnet-targeting-pack-9.0, libcairo, pango, libjpeg-turbo, giflib, librsvg"
CLANDRO_PKG_DEPENDS="aspnetcore-runtime-9.0, dotnet-host, dotnet-runtime-9.0, libskiasharp, libesqlite3, jellyfin-ffmpeg"
CLANDRO_PKG_SERVICE_SCRIPT=(
	"jellyfin"
	"exec ${CLANDRO_PREFIX}/bin/jellyfin 2>&1"
)
...
clandro_step_post_get_source() {
	pushd jellyfin-ffmpeg-"${CLANDRO_PKG_VERSION[1]%.*}-${CLANDRO_PKG_VERSION[1]##*.}"
...
clandro_step_pre_configure() {
	...
	clandro_setup_dotnet; clandro_setup_nodejs

	pushd jellyfin-web-"${CLANDRO_PKG_VERSION[0]}"
	npm install

	# git warning in build log here is normal, the commit hash is not needed
	npm run build:production
	cp -R ./dist "$CLANDRO_PKG_BUILDDIR/jellyfin-web"
	...
	pushd jellyfin-ffmpeg-"${CLANDRO_PKG_VERSION[1]%.*}-${CLANDRO_PKG_VERSION[1]##*.}"
	local _ARCH=""
	local _FFMPEG_PREFIX="${CLANDRO_PREFIX}/opt/jellyfin"
...
clandro_step_make() {
	dotnet publish "$CLANDRO_PKG_SRCDIR"/Jellyfin.Server --configuration Release --runtime "$DOTNET_TARGET_NAME" --output "$CLANDRO_PKG_BUILDDIR"/build --no-self-contained -p:DebugType=None
	dotnet build-server shutdown
}

clandro_step_make_install() {
	# we provide bionic builds of these in the repo
	rm -f "${CLANDRO_PKG_BUILDDIR}/build/"{libe_sqlite3,libSkiaSharp,libHarfBuzzSharp}.so*
	chmod 0700 "${CLANDRO_PKG_BUILDDIR}/build"

	mv "${CLANDRO_PKG_BUILDDIR}/jellyfin-web" "${CLANDRO_PKG_BUILDDIR}/build"
	# XML cruft generated during build used to provide documentation for functions and objects
	find "${CLANDRO_PKG_BUILDDIR}/build" -name '*.xml' -type f -exec rm '{}' +
	find "${CLANDRO_PKG_BUILDDIR}/build" ! \( -name 'jellyfin' -o -type d \) -exec chmod 0600 '{}' \;
	find "${CLANDRO_PKG_BUILDDIR}/build" \( -name 'jellyfin' -o -type d \) -exec chmod 0700 '{}' \;
	mv "${CLANDRO_PKG_BUILDDIR}/build" "${CLANDRO_PREFIX}/lib/jellyfin"
	ln -s "${CLANDRO_PREFIX}/lib/jellyfin/jellyfin" "${CLANDRO_PREFIX}/bin/jellyfin"
}
# References
# - Jellyfin-FFMPEG
...
# Note: All patches for Jellyfin-FFMPEG should be based off the patched version, see clandro_step_post_get_source
