CLANDRO_PKG_HOMEPAGE=https://github.com/electron/electron
CLANDRO_PKG_DESCRIPTION="Build cross-platform desktop apps with JavaScript, HTML, and CSS"
CLANDRO_PKG_LICENSE="MIT, BSD 3-Clause"
CLANDRO_PKG_MAINTAINER="@licy183"
_CHROMIUM_VERSION=142.0.7444.265
CLANDRO_PKG_VERSION=39.8.7
CLANDRO_PKG_SRCURL=git+https://github.com/electron/electron
...
CLANDRO_PKG_BUILD_DEPENDS="electron-host-tools-for-code-oss, libnotify, libffi-static"
...
__tur_setup_depot_tools() {
	...
	export CHROMIUM_BUILDTOOLS_PATH="$CLANDRO_PKG_SRCDIR/buildtools"
...
clandro_step_get_source() {
	...
	gclient sync --with_branch_heads --with_tags --no-history --revision v$CLANDRO_PKG_VERSION
	popd

	# Solve error like `.git/packed-refs is dirty`
	cd "$CLANDRO_PKG_CACHEDIR/tmp-checkout/src"
	git pack-refs --all
	cd electron
	git pack-refs --all

	ln -sfr $CLANDRO_PKG_CACHEDIR/tmp-checkout/src $CLANDRO_PKG_SRCDIR
}
...
clandro_step_post_get_source() {
	...
	local f
	for f in $(find "$CLANDRO_PKG_BUILDER_DIR/../electron-host-tools-for-code-oss/cr-patches" -maxdepth 1 -type f -name *.patch | sort); do
	...
	local f
	for f in $(find "$CLANDRO_PKG_BUILDER_DIR/../electron-host-tools-for-code-oss/electron-patches" -maxdepth 1 -type f -name *.patch | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 < "$f"
	done
	...
	local f
	for f in $(find "$CLANDRO_PKG_BUILDER_DIR/../electron-host-tools-for-code-oss/jumbo-patches" -maxdepth 1 -type f -name *.patch | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 < "$f"
	done
	...
	echo "$CLANDRO_PKG_VERSION" > $CLANDRO_PKG_SRCDIR/electron/ELECTRON_VERSION
}
...
override_electron_version = \"$CLANDRO_PKG_VERSION\"
...
electron_js2c_toolchain = \"$CLANDRO_PKG_CACHEDIR/custom-toolchain:$_v8_toolchain_name\"
...
# Use prebuilt js2c
prebuilt_js2c_binary = \"$CLANDRO_PREFIX/opt/electron-host-tools-for-code-oss/$_v8_toolchain_name/node_js2c\"
" >> $_common_args_file
...
clandro_step_make_install() {
	cd $CLANDRO_PKG_BUILDDIR
	local _install_prefix=$CLANDRO_PREFIX/lib/code-oss
	mkdir -p $_install_prefix

	echo "$CLANDRO_PKG_VERSION" > $CLANDRO_PKG_BUILDDIR/out/Release/version
...
clandro_step_install_license() {
	...
	cp out/Release/LICENSE{,S.chromium.html} $CLANDRO_PREFIX/share/doc/$CLANDRO_PKG_NAME/
}
