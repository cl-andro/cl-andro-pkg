...
CLANDRO_SCRIPTDIR=$(realpath "$(dirname "$0")/../")
...
check_version() {
	...
	[[ -z "$base_commit" ]] && {
		...
		ls -AR "$CLANDRO_SCRIPTDIR/.git/refs/remotes/origin"
		return 1
	} >&2

	# If CLANDRO_PKG_VERSION is an array that changes the formatting.
	local version i=0 error=0 is_array="${CLANDRO_PKG_VERSION@a}"
	printf '%s' "${is_array:+$'ARRAY\n'}"

	for version in "${CLANDRO_PKG_VERSION[@]}"; do
		printf '%s' "${is_array:+$'\t'}"
		...
		version_new="${version}-${CLANDRO_PKG_REVISION:-0}"
		version_old=$(
			unset CLANDRO_PKG_VERSION CLANDRO_PKG_REVISION
			# shellcheck source=/dev/null
			. <(git -P show "${base_commit}:${package_dir}/build.sh" 2> /dev/null)
			# ${CLANDRO_PKG_VERSION[0]} also works fine for non-array versions.
			# Since those resolve in 1 iteration, no higher index is ever attempted to be called.
			echo "${CLANDRO_PKG_VERSION[$i]:-0}-${CLANDRO_PKG_REVISION:-0}"
		)
		...
		# The rest of the checks aren't useful past the first index when $CLANDRO_PKG_VERSION is an array
		...
		if dpkg --compare-versions "$version_new" le "${version_old_is_bad:-$version_old}"; then
			printf '%s\n' \
				...
				"Either 'CLANDRO_PKG_VERSION' or 'CLANDRO_PKG_REVISION'" \
			...
			printf '%s\n' \
				...
				"e.g. CLANDRO_PKG_VERSION=${version_new%-*}+really${version_old%-*}" \
		...
		if [[ "${version_new%-*}" == "${version_old%-*}" && "$new_revision" != "$((old_revision + 1))" ]]; then
			...
			printf '%s\n' "FAILED " \
				"CLANDRO_PKG_REVISION should be incremented sequentially" \
			...
			continue
		# If that check passed the CLANDRO_PKG_VERSION must have changed,
		# in which case CLANDRO_PKG_REVISION should be reset to 0.
		elif [[ "${version_new%-*}" != "${version_old%-*}" && "$new_revision" != "0" ]]; then
			(( error++ )) # Not reset
			printf '%s\n' \
				"FAILED - $version_old -> $version_new" \
				"" \
				"CLANDRO_PKG_VERSION was bumped but CLANDRO_PKG_REVISION wasn't reset." \
				"Please remove the 'CLANDRO_PKG_REVISION=${new_revision}' line." \
				""
			continue
		fi
...
lint_package() {
	...
	for channel in $CLANDRO_PACKAGES_DIRECTORIES; do
		[[ -d "$CLANDRO_SCRIPTDIR/$channel/$package_name" ]] && {
			in_dir="$CLANDRO_SCRIPTDIR/$channel/$package_name"
			break
		}
	done
	(( ! ${#in_dir} )) && {
	...
	(set +e +u
		...
		echo -n "CLANDRO_PKG_HOMEPAGE: "
		if (( ${#CLANDRO_PKG_HOMEPAGE} )); then
			if [[ ! "$CLANDRO_PKG_HOMEPAGE" == 'https://'* ]]; then
		...
		echo -n "CLANDRO_PKG_DESCRIPTION: "
		if (( ${#CLANDRO_PKG_DESCRIPTION} )); then

			if (( ${#CLANDRO_PKG_DESCRIPTION} > 100 )); then
		...
		echo -n "CLANDRO_PKG_LICENSE: "
		if (( ${#CLANDRO_PKG_LICENSE} )); then
			case "$CLANDRO_PKG_LICENSE" in
				*custom*) echo "CUSTOM" ;;
				'non-free') echo "NON-FREE";;
				*) check_package_license "$CLANDRO_PKG_LICENSE" || pkg_lint_error=true
				;;
			esac
		else
			echo "NOT SET"
		...
		echo -n "CLANDRO_PKG_MAINTAINER: "
		if (( ${#CLANDRO_PKG_MAINTAINER} )); then
			echo "PASS"
		else
			echo "NOT SET"
			pkg_lint_error=true
		...
		if (( ${#CLANDRO_PKG_API_LEVEL} )); then
		echo -n "CLANDRO_PKG_API_LEVEL: "

			if [[ "$CLANDRO_PKG_API_LEVEL" == [1-9][0-9] ]]; then
				if (( CLANDRO_PKG_API_LEVEL < 24 )); then
		...
		echo -n "CLANDRO_PKG_VERSION: "
		check_version "$package_script" || pkg_lint_error=true

		if (( ${#CLANDRO_PKG_REVISION} )); then
		echo -n "CLANDRO_PKG_REVISION: "

			if (( CLANDRO_PKG_REVISION > 1 || CLANDRO_PKG_REVISION < 999999999 )); then
				echo "PASS"
			else
		...
		if (( ${#CLANDRO_PKG_SKIP_SRC_EXTRACT} )); then
		echo -n "CLANDRO_PKG_SKIP_SRC_EXTRACT: "

			case "$CLANDRO_PKG_SKIP_SRC_EXTRACT" in
				'true'|'false')
					echo "PASS";;
				*)
		...
		echo -n "CLANDRO_PKG_SRCURL: "
		if (( ${#CLANDRO_PKG_SRCURL} )); then
			for (( i = 0; i < ${#CLANDRO_PKG_SRCURL[@]}; i++ )); do
				url="${CLANDRO_PKG_SRCURL[$i]}"
				(( ${#url} )) || {
					echo "NOT SET (\${CLANDRO_PKG_SRCURL[$i]} has no value)"
					pkg_lint_error=true
					break
				}
				# Example:
			...
			echo -n "CLANDRO_PKG_SHA256: "
			if (( ${#CLANDRO_PKG_SHA256} )); then
				if (( ${#CLANDRO_PKG_SRCURL[@]} == ${#CLANDRO_PKG_SHA256[@]} )); then
					sha256_ok="PASS"

					for sha256 in "${CLANDRO_PKG_SHA256[@]}"; do
					...
					echo "LENGTHS OF 'CLANDRO_PKG_SRCURL' AND 'CLANDRO_PKG_SHA256' ARRAYS ARE NOT EQUAL"
					pkg_lint_error=true
				fi
			elif [[ "${CLANDRO_PKG_SRCURL:0:4}" == 'git+' ]]; then
				echo "NOT SET (acceptable since CLANDRO_PKG_SRCURL is git repo)"
			else
				echo "NOT SET"
				pkg_lint_error=true
			fi
			...
			echo -n "NOT SET"
			if [[ "$CLANDRO_PKG_SKIP_SRC_EXTRACT" != 'true' ]] && ! declare -F clandro_step_extract_package > /dev/null 2>&1; then
				echo " (set CLANDRO_PKG_SKIP_SRC_EXTRACT to 'true' if no sources downloaded)"
				pkg_lint_error=true
			else
				echo " (acceptable since CLANDRO_PKG_SKIP_SRC_EXTRACT is true)"
			fi
		fi

		if (( ${#CLANDRO_PKG_METAPACKAGE} )); then
		echo -n "CLANDRO_PKG_METAPACKAGE: "

			case "$CLANDRO_PKG_METAPACKAGE" in
				'true'|'false')
					echo "PASS";;
				*)
					echo "INVALID (allowed: true / false)"
		...
		if (( ${#CLANDRO_PKG_ESSENTIAL} )); then
		echo -n "CLANDRO_PKG_ESSENTIAL: "

			case "$CLANDRO_PKG_ESSENTIAL" in
				'true'|'false')
					echo "PASS";;
				*)
					echo "INVALID (allowed: true / false)"
		...
		if (( ${#CLANDRO_PKG_NO_STATICSPLIT} )); then
		echo -n "CLANDRO_PKG_NO_STATICSPLIT: "

			case "$CLANDRO_PKG_NO_STATICSPLIT" in
				'true'|'false')
					echo "PASS";;
				*)
					echo "INVALID (allowed: true / false)"
		...
		if (( ${#CLANDRO_PKG_BUILD_IN_SRC} )); then
		echo -n "CLANDRO_PKG_BUILD_IN_SRC: "

			case "$CLANDRO_PKG_BUILD_IN_SRC" in
				'true'|'false')
					echo "PASS";;
				*)
					echo "INVALID (allowed: true / false)"
		...
		if (( ${#CLANDRO_PKG_HAS_DEBUG} )); then
		echo -n "CLANDRO_PKG_HAS_DEBUG: "

			case "$CLANDRO_PKG_HAS_DEBUG" in
				'true'|'false')
					echo "PASS";;
				*)
					echo "INVALID (allowed: true / false)"
		...
		if (( ${#CLANDRO_PKG_PLATFORM_INDEPENDENT} )); then
		echo -n "CLANDRO_PKG_PLATFORM_INDEPENDENT: "

			case "$CLANDRO_PKG_PLATFORM_INDEPENDENT" in
				'true'|'false')
					echo "PASS";;
				*)
					echo "INVALID (allowed: true / false)"
		...
		if (( ${#CLANDRO_PKG_HOSTBUILD} )); then
		echo -n "CLANDRO_PKG_HOSTBUILD: "

			case "$CLANDRO_PKG_HOSTBUILD" in
				'true'|'false')
					echo "PASS";;
				*)
					echo "INVALID (allowed: true / false)"
		...
		if (( ${#CLANDRO_PKG_FORCE_CMAKE} )); then
		echo -n "CLANDRO_PKG_FORCE_CMAKE: "

			case "$CLANDRO_PKG_FORCE_CMAKE" in
				'true'|'false')
					echo "PASS";;
				*)
					echo "INVALID (allowed: true / false)"
		...
		if (( ${#CLANDRO_PKG_RM_AFTER_INSTALL} )); then
		echo -n "CLANDRO_PKG_RM_AFTER_INSTALL: "
			file_path_ok=true
			...
			done <<< "$CLANDRO_PKG_RM_AFTER_INSTALL"
			unset file_path
		...
		if (( ${#CLANDRO_PKG_CONFFILES} )); then
		echo -n "CLANDRO_PKG_CONFFILES: "
			file_path_ok=true
		...
		if (( ${#CLANDRO_PKG_SERVICE_SCRIPT} )); then
		echo -n "CLANDRO_PKG_SERVICE_SCRIPT: "

			if (( ${#CLANDRO_PKG_SERVICE_SCRIPT[@]} % 2 )); then
				echo "INVALID (CLANDRO_PKG_SERVICE_SCRIPT has to be an array of even length)"
				pkg_lint_error=true
			else
				echo "PASS"
			fi
...
linter_main() {
	...
	if [[ "$problems_found" == 'true' ]]; then
		echo "================================================================"
		echo
		echo "A problem has been found in '$(realpath --relative-to="$CLANDRO_SCRIPTDIR" "$package_script")'."
...
if (( $# )); then
	...
	for repo_dir in $(jq --raw-output 'del(.pkg_format) | keys | .[]' "$CLANDRO_SCRIPTDIR/repo.json"); do
