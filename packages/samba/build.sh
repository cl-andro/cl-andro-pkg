CLANDRO_PKG_HOMEPAGE=https://www.samba.org/
CLANDRO_PKG_DESCRIPTION="SMB/CIFS fileserver"
CLANDRO_PKG_LICENSE="GPL-3.0"
CLANDRO_PKG_MAINTAINER="@clandro"
CLANDRO_PKG_VERSION="4.16.11"
CLANDRO_PKG_REVISION=6
CLANDRO_PKG_SRCURL=https://download.samba.org/pub/samba/samba-${CLANDRO_PKG_VERSION}.tar.gz
CLANDRO_PKG_SHA256=5218878cdcc01aa8e83d2c84ad16c5f37a01ea5e1a93f640f9ee282053c46e12
CLANDRO_PKG_DEPENDS="krb5, libandroid-execinfo, libandroid-spawn, libbsd, libcap, libcrypt, libgnutls, libiconv, libicu, libpopt, libtalloc, libtasn1, libtirpc, ncurses, openssl, readline, tdb-tools, zlib"
CLANDRO_PKG_BUILD_DEPENDS="e2fsprogs"
...
Checking uname machine type: "$CLANDRO_ARCH"
...
EOF
	...
	./buildtools/bin/waf configure \
		...
		--without-quotas \
		--without-regedit \
		--with-system-mitkrb5 "$CLANDRO_PREFIX" \
		--without-systemd \
		--without-utmp \
		--without-winbind \
		...
		# --with-logfilebase="$CLANDRO_PREFIX/tmp/log/samba" \
		(cat cross-answers.txt | grep UNKNOWN && return 1)
}


clandro_step_make() {
	./buildtools/bin/waf build --jobs="$CLANDRO_PKG_MAKE_PROCESSES"
}

clandro_step_make_install() {
	./buildtools/bin/waf install --jobs="$CLANDRO_PKG_MAKE_PROCESSES"
}

clandro_step_post_make_install() {
	install -Dm700 -t "$CLANDRO_PREFIX/bin" \
		"$CLANDRO_PKG_SRCDIR/examples/scripts/nmb/findsmb"
	mkdir -p "$CLANDRO_PREFIX/share/doc/samba"
	sed -e "s|@CLANDRO_PREFIX@|${CLANDRO_PREFIX}|g" \
		"$CLANDRO_PKG_BUILDER_DIR/smb.conf.example.in" \
		> "$CLANDRO_PREFIX/share/doc/samba/smb.conf.example"
}
