...
PACKAGES+=" clang"				# Required for clandro-elf-cleaner and C/C++ packages.
PACKAGES+=" file"				# Used in clandro_step_massage().
PACKAGES+=" gnupg"				# Used in clandro_get_repo_files() and build-package.sh.
...
PYTHON_PACKAGES+=" setuptools"			# necessary to build mesa (explicitly 'system'-wide unlike the setuptools in clandro_setup_python_pip)
# More 'system-wide' python packages should be added here if working towards the goal
# of setup-clandro.sh for on-device building having closer behavior
...
export CLANDRO_SCRIPTDIR=$(dirname "$(realpath "$0")")/../
. $(dirname "$(realpath "$0")")/properties.sh
source "$CLANDRO_PREFIX/bin/clandro-setup-package-manager" || true
...
# so should be installed Clandro-wide for on-device building to be reasonably accurate
