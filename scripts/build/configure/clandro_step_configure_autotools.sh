clandro_step_configure_autotools() {
	if [ ! -e "$CLANDRO_PKG_SRCDIR/configure" ]; then return; fi

	local ENABLE_STATIC="--enable-static"
	if [ "$CLANDRO_PKG_EXTRA_CONFIGURE_ARGS" != "${CLANDRO_PKG_EXTRA_CONFIGURE_ARGS/--disable-static/}" ]; then
		ENABLE_STATIC=""
	fi

	local DISABLE_NLS="--disable-nls"
	if [ "$CLANDRO_PKG_EXTRA_CONFIGURE_ARGS" != "${CLANDRO_PKG_EXTRA_CONFIGURE_ARGS/--enable-nls/}" ]; then
		# Do not --disable-nls if package explicitly enables it (for gettext itself)
		DISABLE_NLS=""
	fi

	local ENABLE_SHARED="--enable-shared"
	if [ "$CLANDRO_PKG_EXTRA_CONFIGURE_ARGS" != "${CLANDRO_PKG_EXTRA_CONFIGURE_ARGS/--disable-shared/}" ]; then
		ENABLE_SHARED=""
	fi

	local HOST_FLAG="--host=$CLANDRO_HOST_PLATFORM"
	if [ "$CLANDRO_PKG_EXTRA_CONFIGURE_ARGS" != "${CLANDRO_PKG_EXTRA_CONFIGURE_ARGS/--host=/}" ]; then
		HOST_FLAG=""
	fi

	local LIBEXEC_FLAG="--libexecdir=$CLANDRO_PREFIX/libexec"
	if [ "$CLANDRO_PKG_EXTRA_CONFIGURE_ARGS" != "${CLANDRO_PKG_EXTRA_CONFIGURE_ARGS/--libexecdir=/}" ]; then
		LIBEXEC_FLAG=""
	fi
	...
	if [ "$CLANDRO_ON_DEVICE_BUILD" = "false" ] && [ ! -d "$CLANDRO_PKG_TMPDIR/config-scripts" ]; then
		# Some packages provides a $PKG-config script which some configure scripts pickup instead of pkg-config:
		mkdir "$CLANDRO_PKG_TMPDIR/config-scripts"
		for f in $CLANDRO_PREFIX/bin/*config; do
			if [[ -f "$f" && "$(head -c 4 "$f")" != "$(echo -ne '\0177ELF')" ]]; then
				cp "$f" "$CLANDRO_PKG_TMPDIR/config-scripts"
			fi
		done
		export PATH=$CLANDRO_PKG_TMPDIR/config-scripts:$PATH
	fi
	...
	env $AVOID_GNULIB "$CLANDRO_PKG_SRCDIR/configure" \
		...
		$LIBEXEC_FLAG \
		$QUIET_BUILD \
		|| (clandro_step_configure_autotools_failure_hook && false)
}

clandro_step_configure_autotools_failure_hook() {
	false
}
