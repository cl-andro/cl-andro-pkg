...
CLANDRO_PKG_DESCRIPTION="Tools and libraries to manipulate a wide range of multimedia formats and protocols"
...
clandro_step_pre_configure() {
	...
	for lib in util codec format; do
		...
		if [[ ! "${so_version}"  ||  "${_FFMPEG_SOVER[av${lib}]}" != "${so_version}" ]]; then
			clandro_error_exit "SOVERSION guard check failed for libav${lib}.so. expected ${so_version}"
		fi
	done
}
...
clandro_step_post_massage() {
	cd "${CLANDRO_PKG_MASSAGEDIR}/${CLANDRO_PREFIX}/lib" || clandro_error_exit "couldn't symlink shared libraries."
	local lib so_version
	for lib in util codec format; do
		so_version="${_FFMPEG_SOVER[av${lib}]}"
		if [[ ! "${so_version}" ]]; then
			clandro_error_exit "Empty SOVERSION for libav${lib}."
		fi
