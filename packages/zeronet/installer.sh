...
mkdir -p "${TMPDIR:-@CLANDRO_PREFIX@/tmp}"/.ldconfig
echo -e "#!@CLANDRO_PREFIX@/bin/bash\nexec true" > "${TMPDIR:-@CLANDRO_PREFIX@/tmp}"/.ldconfig/ldconfig
chmod 700 "${TMPDIR:-@CLANDRO_PREFIX@/tmp}"/.ldconfig/ldconfig
export PATH="$PATH:${TMPDIR:-@CLANDRO_PREFIX@/tmp}/.ldconfig"
...
# Cleanup.
rm -rf "${TMPDIR:-@CLANDRO_PREFIX@/tmp}"/.ldconfig
