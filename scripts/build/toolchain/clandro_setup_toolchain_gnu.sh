clandro_setup_toolchain_gnu() {
	...
	export CC=$CLANDRO_HOST_PLATFORM-gcc
	export CXX=$CLANDRO_HOST_PLATFORM-g++
	export AR=$CLANDRO_HOST_PLATFORM-gcc-ar
	export RANLIB=$CLANDRO_HOST_PLATFORM-gcc-ranlib
	export NM=$CLANDRO_HOST_PLATFORM-gcc-nm
	export LD=ld
	export AS=as
	...
	if [ "$CLANDRO_ON_DEVICE_BUILD" = "true" ]; then
		...
		export CXXFILT=$CLANDRO_HOST_PLATFORM-c++filt
	fi

	if [ ! -d "$CLANDRO__PREFIX__LIB_DIR/" ]; then
		clandro_error_exit "glibc library directory was not found ('$CLANDRO__PREFIX__LIB_DIR/')"
	fi
	if [ ! -f "$CLANDRO__PREFIX__LIB_DIR/libgcc_s.so" ] && [ ! -f "$CLANDRO__PREFIX__LIB_DIR/libgcc_s.so.1" ]; then
		clandro_error_exit "libgcc not found, there is a risk of incorrect compiler operation"
	fi
	if [ ! -d "$CLANDRO__PREFIX__BASE_INCLUDE_DIR/" ]; then
		clandro_error_exit "glibc base header directory was not found ('$CLANDRO__PREFIX__BASE_INCLUDE_DIR/')"
	fi
	if [ "$CLANDRO_ARCH" != "$CLANDRO_REAL_ARCH" ] && [ ! -d "$CLANDRO__PREFIX__MULTI_INCLUDE_DIR/" ]; then
		clandro_error_exit "glibc multi header directory was not found ('$CLANDRO__PREFIX__MULTI_INCLUDE_DIR/')"
	fi
	...
	export PATH_DYNAMIC_LINKER="$CLANDRO__PREFIX__BASE_LIB_DIR/$DYNAMIC_LINKER"

	if [ ! -f "$PATH_DYNAMIC_LINKER" ]; then
		clandro_error_exit "glibc dynamic linker was not found ('$PATH_DYNAMIC_LINKER')"
	fi
	...
	case "$CLANDRO_ARCH" in
 	...
 	export CCLANDRO_HOST_PLATFORM=$CLANDRO_HOST_PLATFORM
	...
	if [ "$CLANDRO_ON_DEVICE_BUILD" = "false" ]; then
		local BASE_PATH="$CLANDRO_COMMON_CACHEDIR/BASE_PATH"
		if [ ! -d "$BASE_PATH" ]; then
		...
		if ! tr ':' '\n' <<< "$PATH" | grep -q "^$CLANDRO_PREFIX/bin$"; then
			export PATH="$CLANDRO_PREFIX/bin:$PATH"
		fi
		if ! tr ':' '\n' <<< "$PATH" | grep -q "^$BASE_PATH$"; then
			export PATH="$BASE_PATH:$PATH"
		fi
	fi
	if ! tr ':' '\n' <<< "$PATH" | grep -q "^$CLANDRO_STANDALONE_TOOLCHAIN/bin$"; then
