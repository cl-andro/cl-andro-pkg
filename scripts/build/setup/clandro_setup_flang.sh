clandro_setup_flang() {
    if [ "$CLANDRO_ON_DEVICE_BUILD" = true ]; then
		if [[ "$CLANDRO_APP_PACKAGE_MANAGER" = "apt" && "$(dpkg-query -W -f '${db:Status-Status}\n' flang 2>/dev/null)" != "installed" ]] ||
			[[ "$CLANDRO_APP_PACKAGE_MANAGER" = "pacman" && ! "$(pacman -Q flang 2>/dev/null)" ]]; then
	...
	local __cache_dir="$CLANDRO_COMMON_CACHEDIR"/flang-toolchain-cache
	mkdir -p "$__cache_dir"
...
80e5dff0271cfb557d4925c50b459d54c615362df03d4bf29810c0857ae93762  package-install.tar.bz2
	...
	local _termux_toolchain_name="$(basename "$CLANDRO_STANDALONE_TOOLCHAIN")"

	local FLANG_FOLDER=
	if [ "${CLANDRO_PACKAGES_OFFLINE-false}" = "true" ]; then
		FLANG_FOLDER="$CLANDRO_SCRIPTDIR/build-tools/$_termux_toolchain_name-flang-v$_flang_toolchain_version"
	else
		FLANG_FOLDER="$CLANDRO_COMMON_CACHEDIR/$_termux_toolchain_name-flang-v$_flang_toolchain_version"
	fi
	...
	if [ ! -d "$FLANG_FOLDER" ]; then
		...
		cp -Rf $CLANDRO_STANDALONE_TOOLCHAIN/sysroot $FLANG_FOLDER_TMP/
		...
		cp -Rf $CLANDRO_STANDALONE_TOOLCHAIN/lib/clang/$clang_major_version/lib/* \
		...
		for host_plat in aarch64-linux-android armv7a-linux-androideabi i686-linux-android x86_64-linux-android; do
			...
			#!/usr/bin/env bash
			if [ "\$1" != "-cpp" ] && [ "\$1" != "-fc1" ]; then
				\`dirname \$0\`/flang-new --target=${host_plat}${CLANDRO_PKG_API_LEVEL} -D__ANDROID_API__=$CLANDRO_PKG_API_LEVEL "\$@"
			else
			...
			cp $FLANG_FOLDER_TMP/bin/${host_plat}-flang-new \
				$FLANG_FOLDER_TMP/bin/${host_plat}${CLANDRO_PKG_API_LEVEL}-flang-new
		done

		cp $FLANG_FOLDER_TMP/bin/armv7a-linux-androideabi-flang-new \
			$FLANG_FOLDER_TMP/bin/arm-linux-androideabi-flang-new
		cp $FLANG_FOLDER_TMP/bin/armv7a-linux-androideabi-flang-new \
			$FLANG_FOLDER_TMP/bin/arm-linux-androideabi${CLANDRO_PKG_API_LEVEL}-flang-new
	...
	export FC=$CLANDRO_HOST_PLATFORM-flang-new
	export FCFLAGS="--target=$CCCLANDRO_HOST_PLATFORM -D__ANDROID_API__=$CLANDRO_PKG_API_LEVEL"
}
