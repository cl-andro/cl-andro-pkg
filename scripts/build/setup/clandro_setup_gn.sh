clandro_setup_gn() {
	...
	local GN_TARFILE=$CLANDRO_COMMON_CACHEDIR/gn_$GN_COMMIT.tar.gz
	local GN_SOURCE=https://gn.googlesource.com/gn/+archive/$GN_COMMIT.tar.gz

	if [ "${CLANDRO_PACKAGES_OFFLINE-false}" = "true" ]; then
		GN_FOLDER=$CLANDRO_SCRIPTDIR/build-tools/gn-$GN_COMMIT
	else
		GN_FOLDER=$CLANDRO_COMMON_CACHEDIR/gn-$GN_COMMIT
	fi
	...
	if [ "$CLANDRO_ON_DEVICE_BUILD" = "false" ]; then
		if [ ! -d "$GN_FOLDER" ]; then
			...
			(
				...
				export CC="clang-${CLANDRO_HOST_LLVM_MAJOR_VERSION}"
				export CXX="clang++-${CLANDRO_HOST_LLVM_MAJOR_VERSION}"
				export LD="clang++-${CLANDRO_HOST_LLVM_MAJOR_VERSION}"
		...
		if [[ "$CLANDRO_APP_PACKAGE_MANAGER" = "apt" && "$(dpkg-query -W -f '${db:Status-Status}\n' gn 2>/dev/null)" != "installed" ]] ||
                   [[ "$CLANDRO_APP_PACKAGE_MANAGER" = "pacman" && ! "$(pacman -Q gn 2>/dev/null)" ]]; then
