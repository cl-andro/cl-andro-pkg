CLANDRO_PKG_HOMEPAGE=https://github.com/fathyb/carbonyl
CLANDRO_PKG_DESCRIPTION="Chromium based browser built to run in a terminal (Host tools, NOT FOR TERMUX)"
...
CLANDRO_PKG_SRCURL=(
	https://github.com/fathyb/carbonyl/archive/refs/tags/v$CLANDRO_PKG_VERSION.tar.gz
...
CLANDRO_PKG_DEPENDS="atk, cups, dbus, fontconfig, gtk3, krb5, libc++, libdrm, libevdev, libxkbcommon, libminizip, libnss, libwayland, libx11, mesa, openssl, pango, pulseaudio, zlib"
...
# CLANDRO_PKG_DEPENDS="libdrm, fontconfig"
...
clandro_step_post_get_source() {
	...
	local f
	# Apply diffs
	for f in $(find "$CLANDRO_PKG_BUILDER_DIR/" -maxdepth 1 -type f -name *.diff | sort); do
	...
	pushd chromium/src
		for f in $(find "$CLANDRO_PKG_BUILDER_DIR/patches" -maxdepth 1 -type f -name *.patch | sort); do
			echo "Applying patch: $(basename $f)"
			patch -p1 < "$f"
		done
	popd
	...
	pushd chromium/src
	for f in $(find "$CLANDRO_PKG_SRCDIR/chromium/patches/chromium/" -maxdepth 1 -type f -name *.patch | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 < "$f"
	done
	popd
	...
	for f in $(find "$CLANDRO_PKG_SRCDIR/chromium/patches/skia/" -maxdepth 1 -type f -name *.diff | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 < "$f"
	done
	popd
	...
	for f in $(find "$CLANDRO_PKG_SRCDIR/chromium/patches/webrtc/" -maxdepth 1 -type f -name *.diff | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 < "$f"
	done
	popd
...
clandro_step_configure() {
	cd $CLANDRO_PKG_SRCDIR/chromium/src
	clandro_setup_ninja
	clandro_setup_gn
	...
	# Construct args
	local _clang_base_path="$CLANDRO_HOST_LLVM_BASE_DIR"
	local _host_cc="$_clang_base_path/bin/clang"
	local _host_cxx="$_clang_base_path/bin/clang++"
	local _host_toolchain="$CLANDRO_PKG_CACHEDIR/custom-toolchain:host"
	local _target_cpu _target_sysroot="$CLANDRO_PKG_TMPDIR/sysroot"
...
" >> $_common_args_file
	...
	gn gen $CLANDRO_PKG_BUILDDIR/out/Release --export-compile-commands
...
clandro_step_make() {
	...
	# time ninja -C $CLANDRO_PKG_BUILDDIR/out/Release headless:headless_shell
}
...
clandro_step_make_install() {
	...
	mkdir -p "$CLANDRO_PREFIX/opt/$CLANDRO_PKG_NAME/$cr_v8_toolchain/"
	cp "${v8_tools[@]/#/out/Release/$cr_v8_toolchain/}" "$CLANDRO_PREFIX/opt/$CLANDRO_PKG_NAME/$cr_v8_toolchain/"
	...
	mkdir -p "$CLANDRO_PREFIX/opt/$CLANDRO_PKG_NAME/host/"
	cp "${host_tools[@]/#/out/Release/host/}" "$CLANDRO_PREFIX/opt/$CLANDRO_PKG_NAME/host/"
	...
	cp "${normal_files[@]/#/out/Release/}" "$CLANDRO_PREFIX/opt/$CLANDRO_PKG_NAME/"
}
