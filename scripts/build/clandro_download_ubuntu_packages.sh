...
clandro_download_ubuntu_packages() {
	{
		(( $# )) || {
			echo "clandro_download_ubuntu_packages():"
			echo "Usage: clandro_download_ubuntu_packages [PACKAGES...]"
			echo "You can additionally overwrite the following parameters via environment variables"
			echo "\$VAR [default value]"
			echo ""
			echo "DESTINATION    [\$CLANDRO_PKG_HOSTBUILD_DIR/ubuntu_packages]"
		...
		if [[ "$CLANDRO_ON_DEVICE_BUILD" == "true" ]]; then
			echo "clandro_download_ubuntu_packages(): This function should not be used during on-device building!"
			return 1
		fi
	} 1>&2
	...
	: "${DESTINATION:="$CLANDRO_PKG_HOSTBUILD_DIR/ubuntu_packages"}"
	: "${ARCHITECTURE:=amd64}"
	...
	local cache_directory="${CLANDRO_PKG_TMPDIR}/Packages_ubuntu_${UBUNTU_RELEASE}_${ARCHITECTURE}"
	...
	for package in "$@"; do
		...
		echo "clandro_download_ubuntu_packages(): Downloading '$package'"
		local deb_name
		deb_name="${deb_url##*/}"
		clandro_download "$deb_url" "${CLANDRO_COMMON_CACHEDIR}/${deb_name}" "${package_sha256}"
		mkdir -p "${CLANDRO_PKG_TMPDIR}/${deb_name}"
		ar x "${CLANDRO_COMMON_CACHEDIR}/${deb_name}" --output="${CLANDRO_PKG_TMPDIR}/${deb_name}"
		tar xf "${CLANDRO_PKG_TMPDIR}/${deb_name}"/data.tar.* -C "${DESTINATION}"
	done
