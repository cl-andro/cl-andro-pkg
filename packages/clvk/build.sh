CLANDRO_PKG_HOMEPAGE=https://github.com/kpet/clvk
CLANDRO_PKG_DESCRIPTION="Experimental implementation of OpenCL on Vulkan"
CLANDRO_PKG_LICENSE="Apache-2.0"
CLANDRO_PKG_MAINTAINER="@clandro"
_COMMIT=5db9f9d93b443d9c0f78c112f7df11a46d8f5d91
_COMMIT_DATE=20260429
_COMMIT_TIME=095507
CLANDRO_PKG_VERSION="0.0.20260429.095507"
CLANDRO_PKG_SRCURL=git+https://github.com/kpet/clvk
...
CLANDRO_PKG_RECOMMENDS="ocl-icd"
CLANDRO_PKG_HOSTBUILD=true
CLANDRO_PKG_AUTO_UPDATE=true
CLANDRO_PKG_EXTRA_CONFIGURE_ARGS="
-DCLSPV_EXTERNAL_LIBCLC_DIR=${CLANDRO_PKG_HOSTBUILD_DIR}/libclc
-DCLVK_BUILD_TESTS=ON
-DCLVK_CLSPV_ONLINE_COMPILER=ON
-DCLVK_VULKAN_IMPLEMENTATION=custom
-DLLVM_INCLUDE_BENCHMARKS=OFF
-DLLVM_INCLUDE_EXAMPLES=OFF
-DLLVM_NATIVE_TOOL_DIR=${CLANDRO_PKG_HOSTBUILD_DIR}/llvm/bin
-DVulkan_INCLUDE_DIRS=${CLANDRO_PREFIX}/include
...
clandro_step_host_build() {
	...
	*) clandro_error_exit "Invalid arch: ${CLANDRO_ARCH}" ;;
	...
	cmake \
		-G Ninja \
		-B "${CLANDRO_PKG_HOSTBUILD_DIR}/llvm" \
		-S "${CLANDRO_PKG_SRCDIR}/external/clspv/third_party/llvm/llvm" \
	...
	ninja \
		-C "${CLANDRO_PKG_HOSTBUILD_DIR}/llvm" \
		-j "${CLANDRO_PKG_MAKE_PROCESSES}" \
		llvm-tblgen clang-tblgen

	local _host_clang_base="$CLANDRO_HOST_LLVM_BASE_DIR"
	cmake \
		-G Ninja \
		-B "${CLANDRO_PKG_HOSTBUILD_DIR}/libclc" \
		-S "${CLANDRO_PKG_SRCDIR}/external/clspv/third_party/llvm/libclc" \
	...
	ninja \
		-C "${CLANDRO_PKG_HOSTBUILD_DIR}/libclc" \
		-j "${CLANDRO_PKG_MAKE_PROCESSES}"
}
...
clandro_step_pre_configure() {
	...
	CLANDRO_PKG_EXTRA_CONFIGURE_ARGS+=" -DVulkan_LIBRARIES=${_libvulkan}"
	...
	*) clandro_error_exit "Invalid arch: ${CLANDRO_ARCH}" ;;
	...
	# CLANDRO_DEBUG_BUILD doesnt have middle ground
	#CLANDRO_PKG_EXTRA_CONFIGURE_ARGS+=" -DCMAKE_BUILD_TYPE=RelWithDebInfo"
	#export STRIP=:
}

clandro_step_make_install() {
	# clvk does not have proper install rule yet
	install -Dm644 -t "${CLANDRO_PREFIX}/lib/clvk" "${CLANDRO_PKG_BUILDDIR}/libOpenCL.so"

	echo "${CLANDRO_PREFIX}/lib/clvk/libOpenCL.so" > "${CLANDRO_PKG_TMPDIR}/clvk.icd"
	install -Dm644 -t "${CLANDRO_PREFIX}/etc/OpenCL/vendors" "${CLANDRO_PKG_TMPDIR}/clvk.icd"
}
