clandro_step_massage() {
	...
	if [ -e "share/mime/mime.cache" ]; then
		clandro_error_exit "MIME cache found in package. Please disable \`update-mime-database\`."
	fi
	...
	if [ "$CLANDRO_PACKAGE_LIBRARY" = "bionic" ]; then
		if [ "$CLANDRO_PKG_NO_STRIP" != "true" ] && [ "$CLANDRO_DEBUG_BUILD" = "false" ]; then
			clandro_step_strip_elf_symbols
		fi

		if [ "$CLANDRO_PKG_NO_ELF_CLEANER" != "true" ]; then
			clandro_step_elf_cleaner
		fi
	fi

	if [ "$CLANDRO_PKG_NO_SHEBANG_FIX" != "true" ]; then
		...
		if [[ -n "$CLANDRO_PKG_NO_SHEBANG_FIX_FILES" ]]; then
		...
		prefix_escaped="$(printf "%s" "$CLANDRO_PREFIX_CLASSICAL" | sed -zE -e 's/[][\.|$(){}?+*^]/\\&/g')"
		...
		while IFS= read -rd '' file; do
			...
			# but Clandro increases the limit to `CLANDRO__FILE_HEADER__BUFFER_SIZE = 340` with `clandro-exec` to
			# accommodate for longer `CLANDRO__ROOTFS` as per `CLANDRO__ROOTFS_DIR___MAX_LEN = 86` (check `ExecIntercept.h`).
			...
			# `#!/home/builder/.clandro-build/python3.12-crossenv-prefix-bionic-x86_64/cross/bin/python3.12`
			header_line=""
			...
			if [[ "${#header_line}" -ge 3 && "$header_line" =~ $shebang_regex ]]; then
				...
				if [[ -n "$shebang_match" ]]; then
					...
					sed --follow-symlinks -i -E "1 s@^#\!(.*)/bin/(.*)@#\!$CLANDRO_PREFIX/bin/\2@" "$file"
				fi
			fi
	...
	test ! -z "$CLANDRO_PKG_RM_AFTER_INSTALL" && rm -Rf $CLANDRO_PKG_RM_AFTER_INSTALL
	...
	if [ "$CLANDRO_PACKAGE_LIBRARY" = "glibc" ] && [ "$CLANDRO_PKG_NAME" != "python-glibc" ]; then
		rm -f ./${ADDING_PREFIX}lib/python${CLANDRO_PYTHON_VERSION}/__pycache__/{base64,platform,quopri}.cpython-${CLANDRO_PYTHON_VERSION//./}.pyc
	fi

	# Check so files were actually installed. Exclude
	# share/doc/$CLANDRO_PKG_NAME/ as a license file is always
	# installed there.
	if [ "$(find . -path "./${ADDING_PREFIX}share/doc/$CLANDRO_PKG_NAME" -prune -o -type f -print | head -n1)" = "" ]; then
		if [ -f "$CLANDRO_PKG_SRCDIR"/configure.ac -o -f "$CLANDRO_PKG_SRCDIR"/configure.in ]; then
			clandro_error_exit "No files in package. Maybe you need to run autoreconf -fi before configuring."
		else
			clandro_error_exit "No files in package."
		fi
	fi
	...
	if [ -n "$HARDLINKS" ]; then
		if [ "$CLANDRO_PACKAGE_LIBRARY" = "bionic" ]; then
			clandro_error_exit "Package contains hard links: $HARDLINKS"
			...
			for i in ${!hard_list[@]}; do
				...
				for j in ${hard_list[$i]}; do
					...
					ln -sf "${CLANDRO_PREFIX_CLASSICAL}/${root_file:2}" "${j}"
				done
			done
		fi
	fi
	...
	if [[ -d ./${ADDING_PREFIX}/${CLANDRO__PREFIX__MULTI_INCLUDE_SUBDIR} && -d ${CLANDRO__PREFIX__BASE_INCLUDE_DIR} ]]; then
		local hpath
		for hpath in $(find ./${ADDING_PREFIX}/${CLANDRO__PREFIX__MULTI_INCLUDE_SUBDIR} -type f); do
			local h=$(sed "s|./${ADDING_PREFIX}/${CLANDRO__PREFIX__MULTI_INCLUDE_SUBDIR}/||g" <<< "$hpath")
			if [[ -f "${CLANDRO__PREFIX__BASE_INCLUDE_DIR}/${h}" && \
				"$(md5sum < "${hpath}")" = "$(md5sum < "${CLANDRO__PREFIX__BASE_INCLUDE_DIR}/${h}")" ]]; then
				rm "${hpath}"
			fi
		done
	fi
	...
	if [[ -d ./${ADDING_PREFIX}/${CLANDRO__PREFIX__MULTI_LIB_SUBDIR}/pkgconfig ]]; then
		local pc
		for pc in $(grep -s -r -l "^includedir=.*/${CLANDRO__PREFIX__MULTI_INCLUDE_SUBDIR}" ./${ADDING_PREFIX}/${CLANDRO__PREFIX__MULTI_LIB_SUBDIR}/pkgconfig); do
			local pc_cflags="$(grep '^Cflags:' "${pc}" | awk -F ':' '{printf $2 "\n"}')"
			if ! grep -q ' -I' <<< "${pc_cflags}"; then
				continue
			fi
			local pc_multilib_path="$(grep '^includedir=' "${pc}" | sed "s|${CLANDRO_PREFIX}|\${prefix}|g" | awk -F '{prefix}/include' '{printf $2}')"
			...
			sed -i -e "s|\(^includedir=.*/\)${CLANDRO__PREFIX__MULTI_INCLUDE_SUBDIR}\(.*\)|\1include|g" \
			...
			cp -r "${pc}" "${CLANDRO__PREFIX__MULTI_LIB_DIR}/pkgconfig"
		done
	fi

	# Check for directory "$PREFIX/man" which indicates packaging error.
	if [ -d "./${ADDING_PREFIX}man" ]; then
		clandro_error_exit "Package contains directory \"\$PREFIX/man\" ($CLANDRO_PREFIX/man). Use \"\$PREFIX/share/man\" ($CLANDRO_PREFIX/share/man) instead."
	fi

	# Check for directory "$PREFIX/$PREFIX" which almost always indicates
	# packaging error.
	if [ -d "./${CLANDRO_PREFIX#/}" ]; then
		clandro_error_exit "Package contains directory \"\$PREFIX/\$PREFIX\" ($CLANDRO_PREFIX/${CLANDRO_PREFIX#/})"
	fi
	...
	if [ -d "./${_python_deb_install_layout_dir}" ]; then
		clandro_error_exit "Package contains directory \"\$PREFIX/${_python_deb_install_layout_dir}\" ($CLANDRO_PREFIX/${_python_deb_install_layout_dir})"
	fi
	...
	if [[ "${CLANDRO_PACKAGE_LIBRARY}" == "bionic" ]]; then
		...
		local t0=$(get_epoch)
		local SYMBOLS=$(${READELF} -s $(${CLANDRO_HOST_PLATFORM}-clang -print-libgcc-file-name) | grep -E "FUNC[[:space:]]+GLOBAL[[:space:]]+HIDDEN" | awk '{ print $8 }')
		SYMBOLS+=" $(echo libandroid_{sem_{open,close,unlink},shm{ctl,get,at,dt}})"
		SYMBOLS+=" $(grep "^    [_a-zA-Z0-9]*;" ${CLANDRO_SCRIPTDIR}/scripts/lib{c,dl,m}.map.txt | cut -d":" -f2 | sed -e "s/^    //" -e "s/;.*//")"
		SYMBOLS+=" ${CLANDRO_PKG_EXTRA_UNDEF_SYMBOLS_TO_CHECK}"
		...
		local t0=$(get_epoch)
		local LIBOMP_SO=$(${CLANDRO_HOST_PLATFORM}-clang -print-file-name=libomp.so)
		local LIBOMP_A=$(${CLANDRO_HOST_PLATFORM}-clang -print-file-name=libomp.a)
		...
		if [[ "${numberOfValid}" -gt "${numberOfFiles}" ]]; then
			clandro_error_exit "${numberOfValid} > ${numberOfFiles}"
		fi
		...
		if [[ "${CLANDRO_PKG_UNDEF_SYMBOLS_FILES}" == "all" ]]; then
			echo "INFO: Skipping output result as CLANDRO_PKG_UNDEF_SYMBOLS_FILES=all"
			undef=""
		fi
		...
		if [[ -n "${undef}" ]]; then
			...
			while IFS= read -r file; do
				...
				*) (( e |= 1 )) || : ;;
				...
				done < <(echo "${CLANDRO_PKG_UNDEF_SYMBOLS_FILES}")
				[[ "${CLANDRO_PKG_UNDEF_SYMBOLS_FILES}" == "error" ]] && (( e |= 1 )) || :
			...
			[[ "${c}" -gt "${numberOfValid}" ]] && clandro_error_exit "${c} > ${numberOfValid}"
			[[ $(( e & 2 )) != 0 ]] && clandro_error_exit "Refer above"
		fi
		...
		if [[ -n "${depend_libomp_so}" && "${CLANDRO_PKG_NO_OPENMP_CHECK}" != "true" ]]; then
			...
			clandro_error_exit "Refer above"
		fi
	...
	elif [ "$CLANDRO_PACKAGE_FORMAT" = "pacman" ]; then
...
# Local function called by clandro_step_massage
