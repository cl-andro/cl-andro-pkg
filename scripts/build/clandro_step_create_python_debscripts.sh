clandro_step_create_python_debscripts() {
	if [[ -n "${SUB_PKG_NAME-}" ]]; then
		...
		local _package_python_home="$SUB_PKG_DIR/massage$CLANDRO_PREFIX/lib/python$CLANDRO_PYTHON_VERSION"
		local _package_python_deps="${CLANDRO_SUBPKG_PYTHON_RUNTIME_DEPS//, / }"
	else
		local _package_name="$CLANDRO_PKG_NAME"
		local _package_python_home="$CLANDRO_PKG_MASSAGEDIR$CLANDRO_PREFIX/lib/python$CLANDRO_PYTHON_VERSION"
		local _package_python_deps="${CLANDRO_PKG_PYTHON_RUNTIME_DEPS//, / }"
	fi

	local py_file_in_lib_python="" pip_metadata_file=""

	# if the package does not contain any .py files in
	# $CLANDRO_PREFIX/lib/python$CLANDRO_PYTHON_VERSION,
	...
	# the software marks itself as depending on from PyPi that are not already installed from other Clandro packages.
	...
	# if there are no .py files in $CLANDRO_PREFIX/lib/python$CLANDRO_PYTHON_VERSION/,
	...
	if [[ ! -f postinst ]]; then
		echo "#!${CLANDRO_PREFIX_CLASSICAL}/bin/sh" >postinst
		chmod 0755 postinst
	...
	if [[ -n "${_package_python_deps}" ]]; then
		...
		cat <<-POSTINST_EOF >>postinst
			...
			LD_PRELOAD='' LDFLAGS="-lpython$CLANDRO_PYTHON_VERSION" MATHLIB="m" "${CLANDRO_PREFIX}/bin/pip3" install ${upgrade_flag} ${_package_python_deps}
		POSTINST_EOF

		# ensure that pip is added as a dependency to all
		# packages that run the 'pip' command during installation.
		if ([[ "$CLANDRO_PACKAGE_FORMAT" == "debian" ]] && ! grep -q -E "Depends.*$pip_package_name(,|$)" control) || ([[ "$CLANDRO_PACKAGE_FORMAT" == "pacman" ]] && ! grep -q "depend = $pip_package_name" .PKGINFO); then
			clandro_error_exit "'$_package_name' must depend on '$pip_package_name' because it needs to run 'pip' during installation!"
		fi
	fi
	...
	cat <<-POSTINST_EOF >>postinst
		if [ -f "${CLANDRO_PREFIX}/bin/py3compile" ]; then
			LD_PRELOAD='' "${CLANDRO_PREFIX}/bin/py3compile" -p "$_package_name" "${CLANDRO_PREFIX}/lib/python${CLANDRO_PYTHON_VERSION}/"
		fi
	POSTINST_EOF
	...
	# prevent execution of the additional commands
	if [[ ! -f prerm ]]; then
		echo "#!${CLANDRO_PREFIX_CLASSICAL}/bin/sh" >prerm
		chmod 0755 prerm
	...
	cat <<-PRERM_EOF >>prerm
		if [ -f "${CLANDRO_PREFIX}/bin/py3clean" ]; then
			LD_PRELOAD='' "${CLANDRO_PREFIX}/bin/py3clean" -p "$_package_name"
		fi
	PRERM_EOF
	...
	if [[ "$CLANDRO_PACKAGE_FORMAT" == "pacman" ]] && ! grep -qs 'post_install' postupg; then
