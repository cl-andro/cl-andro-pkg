...
CLANDRO_PKG_DESCRIPTION="Build cross-platform desktop apps with JavaScript, HTML, and CSS (Used by Code-OSS, Host Tools)"
CLANDRO_PKG_LICENSE="MIT, BSD 3-Clause"
CLANDRO_PKG_MAINTAINER="@licy183"
_CHROMIUM_VERSION=142.0.7444.265
CLANDRO_PKG_VERSION=39.8.8
...
CLANDRO_PKG_BUILD_DEPENDS="libnotify, libffi-static"
...
clandro_step_post_get_source() {
	...
	local f
	for f in $(find "$CLANDRO_PKG_BUILDER_DIR/cr-patches" -maxdepth 1 -type f -name *.patch | sort); do
	...
	local f
	for f in $(find "$CLANDRO_PKG_BUILDER_DIR/electron-patches" -maxdepth 1 -type f -name *.patch | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 < "$f"
	done
	...
	local f
	for f in $(find "$CLANDRO_PKG_BUILDER_DIR/jumbo-patches" -maxdepth 1 -type f -name *.patch | sort); do
		echo "Applying patch: $(basename $f)"
		patch --silent -p1 < "$f"
	done
...
# Use prebuilt js2c
# prebuilt_js2c_binary = \"$CLANDRO_PREFIX/opt/electron-jumbo-host-tools/$_v8_toolchain_name/node_js2c\"
" >> $_common_args_file
...
clandro_step_make_install() {
	...
	echo "$CLANDRO_PKG_VERSION" > $_install_prefix/version
	...
	rm -rf $CLANDRO_PREFIX/lib/code-oss/node_headers
	cp -Rf out/Release/gen/node_headers $CLANDRO_PREFIX/lib/code-oss/
