...
export CLANDRO_PACKAGE_MANAGER="@CLANDRO_PACKAGE_MANAGER@"
export CLANDRO_PACKAGE_ARCH="@CLANDRO_PACKAGE_ARCH@"

CLANDRO__USER_ID___N="@CLANDRO_ENV__S_CLANDRO@USER_ID"
CLANDRO__USER_ID="${!CLANDRO__USER_ID___N:-}"
...
show_help() {
	...
	cat <<'HELP_EOF'
@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@ runs the second stage of Termux
bootstrap installation.


Usage:
  @CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@
...
The Clandro app runs the bootstrap installion first stage by extracting
...
`postinst`. Therefore, `@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@` is run after
...
`@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@` will exit with exit code `0`,
...
`@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@.lock` file as a symlink in the
same directory as `@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@` file as that is
an atomic operation and only the first instance of
`@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@` that creates it will be able to run
...
the lock file manually and run `@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@`
again.
...
run_bootstrap_second_stage() {
	...
	ensure_running_with_clandro_uid || return $?


	output="$(ln -s "@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@" \
		"@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR@/@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@.lock" 2>&1)"
	return_value=$?
	if [ $return_value -ne 0 ]; then
		if [ $return_value -eq 1 ] && [[ "$output" == *"File exists"* ]]; then
			log "The clandro bootstrap second stage has already been run before and cannot be run again."
...
then delete the '@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR@/@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@.lock' \
file manually and run '@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@' again."
			return 0
		else
			log_error "$output"
			log_error "Failed to create lock file for clandro bootstrap second stage at \
'@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_DIR@/@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@.lock'"
	...
	log "Running clandro bootstrap second stage"
	run_bootstrap_second_stage_inner
	return_value=$?
	if [ $return_value -ne 0 ]; then
		log_error "Failed to run clandro bootstrap second stage"
		return $return_value
	fi

	log "The clandro bootstrap second stage completed successfully"
...
run_package_postinst_maintainer_scripts() {
	...
	if [ "${CLANDRO_PACKAGE_MANAGER}" = "apt" ]; then
		...
		if [ -d "${CLANDRO_PREFIX}/var/lib/dpkg/info" ]; then
			local dpkg_version
			...
			for script_path in "${CLANDRO_PREFIX}/var/lib/dpkg/info/"*.postinst; do
				...
				(
					...
					export DPKG_MAINTSCRIPT_ARCH="$CLANDRO_PACKAGE_ARCH"
					...
					export DPKG_ADMINDIR="${CLANDRO_PREFIX}/var/lib/dpkg"
					export DPKG_ROOT=""
	...
	elif [ ${CLANDRO_PACKAGE_MANAGER} = "pacman" ]; then
		...
		if [ -d "${CLANDRO_PREFIX}/var/lib/pacman/local" ]; then
			# Package install files exist at `/var/lib/pacman/local/package-version/install`
			for script_path in "${CLANDRO_PREFIX}/var/lib/pacman/local/"*/install; do
...
ensure_running_with_clandro_uid() {
	...
	if [ $return_value -ne 0 ]; then
		log_error "$uid"
		log_error "Failed to get uid of the user running the '@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@' script"
...
of the @CLANDRO_APP__NAME@ app APK that is not compatible with the device."
			log_error "Uninstall and reinstall the correct APK variant of the @CLANDRO_APP__NAME@ app."
...
ABI/architecture of the device."
	...
	if [[ -n "$CLANDRO__UID" ]] && [[ "$uid" != "$CLANDRO__UID" ]]; then
		log_error "@CLANDRO_BOOTSTRAP__BOOTSTRAP_SECOND_STAGE_ENTRY_POINT_SUBFILE@ cannot be run as the uid '$uid' and \
it must be run as the CLANDRO__UID '$CLANDRO__UID' exported by the @CLANDRO_APP__NAME@ app."
		return 1
	fi
