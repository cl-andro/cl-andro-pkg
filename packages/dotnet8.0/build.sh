...
CLANDRO_PKG_DESCRIPTION=".NET 8.0"
CLANDRO_PKG_LICENSE="MIT"
CLANDRO_PKG_MAINTAINER="@truboxl"
CLANDRO_PKG_VERSION="8.0.26"
_DOTNET_SDK_VERSION="8.0.126"
CLANDRO_PKG_SRCURL=git+https://github.com/dotnet/dotnet
CLANDRO_PKG_GIT_BRANCH="v${_DOTNET_SDK_VERSION}"
CLANDRO_PKG_BUILD_DEPENDS="krb5, libicu, openssl, zlib"
CLANDRO_PKG_SUGGESTS="dotnet-sdk-8.0"
...
clandro_step_pre_configure() {
	...
	CLANDRO_PKG_DEPENDS="aspnetcore-runtime-8.0, dotnet-host, dotnet-runtime-8.0"
	...
	local NODEJS_FOLDER="${CLANDRO_PKG_CACHEDIR}/nodejs-${NODEJS_VERSION}"
	local NODEJS_TAR_XZ="${CLANDRO_PKG_CACHEDIR}/node.tar.xz"
	clandro_download \
	...
	if [[ "$(node --version)" != "v${NODEJS_VERSION}" ]]; then
		clandro_error_exit "$(command -v node) $(node --version) != ${NODEJS_VERSION}"
	fi
}
...
clandro_step_configure() {
	...
	i686) arch=x86 ;;
	x86_64) arch=x64 ;;
	*) clandro_error_exit "Unknown arch: ${CLANDRO_ARCH}"
	esac
	...
	export ANDROID_NDK_ROOT="${CLANDRO_PKG_TMPDIR}"

	# unified sysroot needed when CMAKE_SYSROOT / --sysroot cannot be used
	export ROOTFS_DIR="${CLANDRO_PKG_TMPDIR}/sysroot"
	if [[ -e "${CLANDRO_STANDALONE_TOOLCHAIN}/sysroot.tmp" ]]; then
		rm -f "${CLANDRO_STANDALONE_TOOLCHAIN}/sysroot"
		mv -v "${CLANDRO_STANDALONE_TOOLCHAIN}"/sysroot{.tmp,}
	fi
	rm -fr "${ROOTFS_DIR}"
	echo "INFO: Copying ${CLANDRO_STANDALONE_TOOLCHAIN}/sysroot to ${ROOTFS_DIR}"
	cp -fr "${CLANDRO_STANDALONE_TOOLCHAIN}/sysroot" "${ROOTFS_DIR}"
	echo "INFO: Copying ${CLANDRO_PREFIX} to ${ROOTFS_DIR}"
	cp -fr "${CLANDRO_PREFIX}" "${ROOTFS_DIR}"
	mv -v "${CLANDRO_STANDALONE_TOOLCHAIN}"/sysroot{,.tmp}
	ln -sv "${ROOTFS_DIR}" "${CLANDRO_STANDALONE_TOOLCHAIN}/sysroot"

	#echo "RID=android.${CLANDRO_PKG_API_LEVEL}-${arch}" > "${ROOTFS_DIR}/android_platform"

	# manual clandro_step_configure_cmake
	...
	cat <<- EOL > "${CLANDRO_PKG_TMPDIR}/build/cmake/android.toolchain.cmake"
	set(CMAKE_C_FLAGS "\${CMAKE_C_FLAGS} ${CFLAGS}")
	set(CMAKE_CXX_FLAGS "\${CMAKE_CXX_FLAGS} ${CXXFLAGS}")
	set(CMAKE_SYSROOT "${ROOTFS_DIR}")
	set(CMAKE_C_COMPILER "${CLANDRO_STANDALONE_TOOLCHAIN}/bin/${CC}")
	set(CMAKE_CXX_COMPILER "${CLANDRO_STANDALONE_TOOLCHAIN}/bin/${CXX}")
	set(CMAKE_AR "$(command -v ${AR})")
	set(CMAKE_UNAME "$(command -v uname)")
	set(CMAKE_RANLIB "$(command -v ${RANLIB})")
	set(CMAKE_STRIP "$(command -v ${STRIP})")
	set(CMAKE_FIND_ROOT_PATH "${CLANDRO_PREFIX}")
	set(CMAKE_FIND_ROOT_PATH_MODE_PROGRAM "NEVER")
	set(CMAKE_FIND_ROOT_PATH_MODE_INCLUDE "ONLY")
	set(CMAKE_FIND_ROOT_PATH_MODE_LIBRARY "ONLY")
	#set(CMAKE_INSTALL_PREFIX "${CLANDRO_PREFIX}")
	#set(CMAKE_INSTALL_LIBDIR "${CLANDRO_PREFIX}/lib")
	set(CMAKE_SKIP_INSTALL_RPATH "ON")
	set(CMAKE_USE_SYSTEM_LIBRARIES "True")
	set(CMAKE_CROSSCOMPILING "True")
	set(DOXYGEN_EXECUTABLE "")
	set(BUILD_TESTING "OFF")
	set(CMAKE_LINKER "${CLANDRO_STANDALONE_TOOLCHAIN}/bin/${LD}")
	set(CMAKE_SYSTEM_NAME "Android")
	set(CMAKE_SYSTEM_VERSION "${CLANDRO_PKG_API_LEVEL}")
	set(CMAKE_SYSTEM_PROCESSOR "${CMAKE_PROC}")
	set(CMAKE_ANDROID_STANDALONE_TOOLCHAIN "${CLANDRO_STANDALONE_TOOLCHAIN}")
	...
	echo "INFO: ${CLANDRO_PKG_TMPDIR}/build/cmake/android.toolchain.cmake"
	cat "${CLANDRO_PKG_TMPDIR}/build/cmake/android.toolchain.cmake"
	echo
...
clandro_step_make() {
	...
	time ./build.sh \
		...
		--online \
		-- \
		-m:${CLANDRO_PKG_MAKE_PROCESSES} \
		/p:Configuration=${CONFIG} \
		/p:OverrideTargetRid=linux-bionic-${arch}

	"${CLANDRO_PKG_BUILDDIR}/.dotnet/dotnet" build-server shutdown
	clandro_dotnet_kill
}

clandro_step_make_install() {
	local _DOTNET_ROOT="${CLANDRO_PREFIX}/lib/dotnet"
	rm -fr "${_DOTNET_ROOT}"
	mkdir -p "${_DOTNET_ROOT}"

	# DEBUG copy the artifacts
	#mkdir -p "${CLANDRO_PREFIX}/opt/${CLANDRO_PKG_NAME}"
	#find "${CLANDRO_PKG_BUILDDIR}/artifacts/x64" -type f \( -name "*.tar.gz" -o -name "*.zip" \) -exec cp -fv "{}" "${CLANDRO_PREFIX}/opt/${CLANDRO_PKG_NAME}" \;

	# TODO fix hardcode in source, fixed in dotnet9.0
	# extract tarball
	tar -xf "${CLANDRO_PKG_BUILDDIR}/artifacts/x64/${CONFIG}"/dotnet-sdk-*.tar.gz -C "${_DOTNET_ROOT}"
	tar -xf "${CLANDRO_PKG_BUILDDIR}/artifacts/x64/${CONFIG}"/dotnet-symbols-sdk-*.tar.gz -C "${_DOTNET_ROOT}"
	...
	cat <<-EOL > "${CLANDRO_PREFIX}/bin/dotnet"
	#!${CLANDRO_PREFIX}/bin/sh
	LD_LIBRARY_PATH="\${LD_LIBRARY_PATH}:${CLANDRO_PREFIX}/lib" exec ${_DOTNET_ROOT}/dotnet "\$@"
	EOL
	chmod 0755 "${CLANDRO_PREFIX}/bin/dotnet"

	# https://src.fedoraproject.org/rpms/dotnet8.0/raw/rawhide/f/dotnet.sh.in
	mkdir -p "${CLANDRO_PREFIX}/etc/profile.d"
	sed \
		-e "s|@LIBDIR@|${CLANDRO_PREFIX}/lib|g" \
		"${CLANDRO_PKG_BUILDER_DIR}/dotnet.sh.in" \
		> "${CLANDRO_PREFIX}/etc/profile.d/dotnet.sh"

	# shell completion
	install -Dm644 "${CLANDRO_PKG_SRCDIR}/src/sdk/scripts/register-completions.bash" "${CLANDRO_PREFIX}/share/bash-completion/completions/dotnet"
	install -Dm644 "${CLANDRO_PKG_SRCDIR}/src/sdk/scripts/register-completions.zsh" "${CLANDRO_PREFIX}/share/zsh/site-functions/_dotnet"

	# manpages
	install -Dm644 -t "${CLANDRO_PREFIX}"/share/man/man1 \
		"${CLANDRO_PKG_SRCDIR}"/src/sdk/documentation/manpages/sdk/*.1
	...
	find \
		...
		> "${CLANDRO_PKG_TMPDIR}"/aspnetcore-runtime.txt

	find \
		lib/dotnet/shared/Microsoft.AspNetCore.App/8.0.* \
		\( -type f -o -type l \) -name "*.pdb" | sort \
		> "${CLANDRO_PKG_TMPDIR}"/aspnetcore-runtime-dbg.txt

	find \
		lib/dotnet/packs/Microsoft.AspNetCore.App.Ref/8.0.* \
		\( -type f -o -type l \) | sort \
		> "${CLANDRO_PKG_TMPDIR}"/aspnetcore-targeting-pack.txt

	find \
		lib/dotnet/packs/Microsoft.NETCore.App.Host.linux-bionic-${arch}/8.0.* \
		\( -type f -o -type l \) | sort \
		> "${CLANDRO_PKG_TMPDIR}"/dotnet-apphost-pack.txt
	...
	find \
		...
		\( -type f -o -type l \) | sort \
		> "${CLANDRO_PKG_TMPDIR}"/dotnet-host.txt
	echo "share/man/man1" >> "${CLANDRO_PKG_TMPDIR}"/dotnet-host.txt

	find \
		lib/dotnet/host/fxr/8.0.* \
		\( -type f -o -type l \) | sort \
		> "${CLANDRO_PKG_TMPDIR}"/dotnet-hostfxr.txt

	find \
		lib/dotnet/shared/Microsoft.NETCore.App/8.0.* \
		\( -type f -o -type l \) ! -name "*.pdb" | sort \
		> "${CLANDRO_PKG_TMPDIR}"/dotnet-runtime.txt

	find \
		lib/dotnet/shared/Microsoft.NETCore.App/8.0.* \
		\( -type f -o -type l \) -name "*.pdb" | sort \
		> "${CLANDRO_PKG_TMPDIR}"/dotnet-runtime-dbg.txt
	...
	find \
		...
		\( -type f -o -type l \) ! -name "*.pdb" | sort \
		> "${CLANDRO_PKG_TMPDIR}"/dotnet-sdk.txt
	...
	find \
		...
		lib/dotnet/sdk/8.0.* \
		\( -type f -o -type l \) -name "*.pdb" | sort \
		> "${CLANDRO_PKG_TMPDIR}"/dotnet-sdk-dbg.txt

	find \
		lib/dotnet/packs/Microsoft.NETCore.App.Ref/8.0.* \
		\( -type f -o -type l \) | sort \
		> "${CLANDRO_PKG_TMPDIR}"/dotnet-targeting-pack.txt

	find \
		lib/dotnet/templates/8.0.* \
		\( -type f -o -type l \) | sort \
		> "${CLANDRO_PKG_TMPDIR}"/dotnet-templates.txt

	find \
		lib/dotnet/packs/NETStandard.Library.Ref \
		\( -type f -o -type l \) | sort \
		> "${CLANDRO_PKG_TMPDIR}"/netstandard-targeting-pack-2.1.txt
	popd

	local txt
	for txt in "${CLANDRO_PKG_TMPDIR}"/*.txt; do
		echo "INFO: $txt"
		cat "$txt"
	done
}
...
clandro_step_post_make_install() {
	...
	rm -fr "${CLANDRO_STANDALONE_TOOLCHAIN}/sysroot"
	mv -v "${CLANDRO_STANDALONE_TOOLCHAIN}"/sysroot{.tmp,}
...
clandro_step_post_massage() {
	local _microsoft_netcore_app_dir="${CLANDRO_PREFIX}/lib/dotnet/shared/Microsoft.NETCore.App"
	...
	for _rpath_check_file in libSystem.Security.Cryptography.Native.OpenSsl.so libcoreclr.so libSystem.Net.Security.Native.so; do
		if [[ ! -f "${_microsoft_netcore_app_dir}/${CLANDRO_PKG_VERSION}/${_rpath_check_file}" ]]; then
			echo "ERROR: ${_microsoft_netcore_app_dir}/${CLANDRO_PKG_VERSION}/${_rpath_check_file} does not exist!"
			...
			clandro_error_exit "Please review error above!"
		fi
		local _rpath_check_readelf=$("$READELF" -d "${_microsoft_netcore_app_dir}/${CLANDRO_PKG_VERSION}/${_rpath_check_file}")
		local _rpath=$(echo "${_rpath_check_readelf}" | sed -ne "s|.*RUNPATH.*\[\(.*\)\].*|\1|p")
		if [[ "${_rpath}" != "${CLANDRO_PREFIX}/lib" ]]; then
			clandro_error_exit "
